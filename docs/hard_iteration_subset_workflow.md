...
- vanilla one-shot summaries under `exp/hard_iteration_one_shot/`
...
`feat/hard-iteration-subset-qd`, but the live data-collection stages remain
blocked until the shared vLLM endpoint at
`http://host.docker.internal:8000/v1/models` becomes reachable again.
