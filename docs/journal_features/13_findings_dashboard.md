...
| M13 | **`validate_journal_revamp_run.py` mechanically CONFIRMS descriptor degeneracy on QD finals.** classic seed_1001 PASSES (coverage 13/13, config+seed snapshot, scheduler telemetry occ 0.39); qd_target seed_1001 FAILS (exit 1) — fatal descriptor collapse on 4/13 problems: adder_8bit / circuit7 / fsmonehot (all 3 axes collapsed, occupied_cells=1) and gshare (occupied_cells=0). This is the SAME descriptor-degeneracy the root-cause dossier + F12 characterize, now confirmed by the mechanical gate. **Consequence:** the verification-surface item "validate exit 0 per run root" is met for classic but NOT for QD — disclosed as the characterized phenomenon (the collapse IS the finding), not a hidden gate failure. Artifacts: `exp/ablation_matrix/stats/validate_{classic,qd_target}_1001`. | `MEASURED` [H: 2026-06-16] |
