...
- frozen smoke, representative, suite, and retirement rules;
...
- smoke, representative, and full-suite reports reached by the candidate;
...
5. Run the frozen representative probe with matched seeds.
6. Advance a sound, non-catastrophic mechanism to a two-seed full-suite probe.
...
- all registered metrics and paired units are reported;
- candidate-specific causal telemetry is analyzed;
- no treatment-based task filtering or metric substitution occurs;
- retirement uses only frozen catastrophic, mechanism, or review failures.
...
A terminal `VIABLE` result requires this full-suite gate. Representative-only
evidence may prioritize work but cannot license a terminal positive claim.

## Terminal Outcomes

- `PAPER_CANDIDATE`: satisfies its role-specific confirmation and holdout gates
  from `../program_claims_contract.md`.
