...
Completed the second seed-3 main-screening command for landing
Smooth-QD manual-BD seed 1003.
...
- seed: 1003
- benchmark group: `VerilogEval-Spec-to-RTL`
- run time: 2201.81 seconds
- run log:
  `exp/auto_bd_research/main_screening_screening_seed3/landing_smooth_qd_manual_bd/seed_1003/revolution/openai_gpt-oss-120b/20260619_063507_revolution_run_log.txt`
- summary:
  `exp/auto_bd_research/main_screening_screening_seed3/landing_smooth_qd_manual_bd/seed_1003/revolution/openai_gpt-oss-120b/20260619_063507_revolution_summary_results.txt`
- scheduler telemetry:
  `exp/auto_bd_research/main_screening_screening_seed3/landing_smooth_qd_manual_bd/seed_1003/revolution/openai_gpt-oss-120b/20260619_063507_revolution_scheduler_telemetry.json`
...
| `Prob135_m2014_q6b` | `success` | 0.26414050822122576 |
| `Prob150_review2015_fsmonehot` | `success` | 0.32968627450980387 |
| `Prob151_review2015_fsm` | `failed` | N/A |
| `Prob153_gshare` | `success` | 0.1796310515178832 |
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 12 complete, 12 pending
- arm/seed pairs: 5 standard-results complete, 1 runs complete, 6 pending
- `landing_smooth_qd_manual_bd` seed 1003: runs complete, 2/2 benchmark groups complete
...
payload = json.loads(p.read_text())
assert payload["manifest_summary"] == {"complete": 12, "total": 12}
assert payload["entry_summary"] == {"complete": 12, "pending": 12, "total": 24}
assert payload["arm_seed_summary"] == {
    "pending": 6,
    "runs_complete": 1,
    "standard_results_complete": 5,
...
entry = next(
    ...
    if item["arm_name"] == "landing_smooth_qd_manual_bd"
    and item["seed"] == 1003
)
assert entry["status"] == "runs_complete"
assert entry["benchmark_groups_complete"] == 2
...
print("landing smooth seed1003 verilog status check ok")
PY
```

Result: `landing smooth seed1003 verilog status check ok`.

## 2026-06-19 07:16 UTC

Generated standard results for `landing_smooth_qd_manual_bd` seed 1003 in
the seed-3 main-screening matrix.
...
UV_LINK_MODE=copy uv run --active python \
  scripts/build_auto_bd_standard_results.py \
  --run-dir exp/auto_bd_research/main_screening_screening_seed3/landing_smooth_qd_manual_bd/seed_1003/revolution/openai_gpt-oss-120b \
  --output-dir exp/auto_bd_research/main_screening_screening_seed3/landing_smooth_qd_manual_bd/seed_1003/standard_results \
  --method-name landing_smooth_qd_manual_bd \
  --method-family smooth_qd_manual_bd \
  --descriptor-version journal_logic_ff_width_3d \
  --phase main_screening \
  --seed 1003 \
  --run-manifest exp/auto_bd_research/main_screening_screening_seed3/landing_smooth_qd_manual_bd/seed_1003/run_manifest.json
```

Generated standard-result files:
...
- candidate count: 1560
- valid PPA candidate count: 799
- unique canonical netlist count: 320
- unique motif signature count: 226
- common audit occupied cells: 40 / 256
- common audit QD score: 8.434309297842587
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 12 complete, 12 pending
- arm/seed pairs: 6 standard-results complete, 6 pending
- `landing_smooth_qd_manual_bd` seed 1003: standard results complete
...
base = Path(
    "exp/auto_bd_research/main_screening_screening_seed3/"
    "landing_smooth_qd_manual_bd/seed_1003/standard_results"
)
required = {
    "archive_snapshots.parquet",
    "candidates.parquet",
...
assert {p.name for p in base.iterdir()} == required
summary = json.loads((base / "method_summary.json").read_text())
assert summary["candidate_count"] == 1560
assert summary["valid_ppa_candidate_count"] == 799
assert summary["unique_canonical_netlist_count"] == 320
assert summary["unique_motif_signature_count"] == 226
assert summary["common_audit_occupied_cells"] == 40
assert summary["common_audit_qd_score"] == 8.434309297842587
...
status = json.loads(status_path.read_text())
assert status["entry_summary"] == {"complete": 12, "pending": 12, "total": 24}
assert status["arm_seed_summary"] == {
    "pending": 6,
    "standard_results_complete": 6,
    "total": 12,
}
entry = next(
    item
...
assert entry["status"] == "standard_results_complete"
print("landing smooth seed1003 standard results check ok")
PY
```

Result: `landing smooth seed1003 standard results check ok`.

## 2026-06-19 07:53 UTC

Completed the first seed-3 main-screening command for
`random_descriptor_qd` seed 1001.
...
env PYTHONPATH=src /workspace/.venv/bin/python scripts/run_backend.py \
  ...
  --post_synthesis_simulation_timeout_s 300 \
  --seed 1001 \
  --save_path /workspace/.worktrees/journal-auto-bd-exp-20260618/exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1001 \
  --search_mode revolution_qd \
  --qd_archive_type grid_quantile \
  --qd_grid_quantile_warmup_successes 8 \
  --qd_cell_mode pareto_front \
...
- seed: 1001
- benchmark group: `RTLLM`
- run time: 2081.15 seconds
- run log:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1001/revolution/openai_gpt-oss-120b/20260619_071756_revolution_run_log.txt`
- summary:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1001/revolution/openai_gpt-oss-120b/20260619_071756_revolution_summary_results.txt`
- scheduler telemetry:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1001/revolution/openai_gpt-oss-120b/20260619_071756_revolution_scheduler_telemetry.json`
...
| `Prob004_adder_8bit` | `success` | 0.38154412826809186 |
| `Prob015_multi_pipe_8bit` | `success` | 0.22434044941785722 |
| `Prob024_fsm` | `success` | 0.66612657699927 |
| `Prob037_parallel2serial` | `success` | 0.08458997628975556 |
| `Prob041_traffic_light` | `success` | 0.4422688211086904 |
| `Prob045_alu` | `success` | 0.40941260923845196 |
| `Prob049_signal_generator` | `success` | 0.3047895385391292 |
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 13 complete, 11 pending
- arm/seed pairs: 6 standard-results complete, 1 partial, 5 pending
- `random_descriptor_qd` seed 1001: partial, 1/2 benchmark groups complete
...
payload = json.loads(p.read_text())
assert payload["manifest_summary"] == {"complete": 12, "total": 12}
assert payload["entry_summary"] == {"complete": 13, "pending": 11, "total": 24}
assert payload["arm_seed_summary"] == {
    "partial": 1,
    "pending": 5,
    "standard_results_complete": 6,
...
print("random descriptor seed1001 rttlm status check ok")
PY
```

Result: `random descriptor seed1001 rttlm status check ok`.
...
- seed: 1001
- benchmark group: `VerilogEval-Spec-to-RTL`
- run time: 2382.75 seconds
- run log:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1001/revolution/openai_gpt-oss-120b/20260619_075512_revolution_run_log.txt`
- summary:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1001/revolution/openai_gpt-oss-120b/20260619_075512_revolution_summary_results.txt`
- scheduler telemetry:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1001/revolution/openai_gpt-oss-120b/20260619_075512_revolution_scheduler_telemetry.json`
...
| `Prob135_m2014_q6b` | `success` | 0.26414050822122576 |
| `Prob150_review2015_fsmonehot` | `success` | 0.32968627450980387 |
| `Prob151_review2015_fsm` | `success` | -0.2443950531198853 |
| `Prob153_gshare` | `success` | 0.15709295997209458 |
...
- arm/seed pairs: 6 standard-results complete, 1 runs complete, 5 pending
- `random_descriptor_qd` seed 1001: runs complete, 2/2 benchmark groups complete
...
payload = json.loads(p.read_text())
assert payload["manifest_summary"] == {"complete": 12, "total": 12}
assert payload["entry_summary"] == {"complete": 14, "pending": 10, "total": 24}
assert payload["arm_seed_summary"] == {
    "pending": 5,
    "runs_complete": 1,
    "standard_results_complete": 6,
...
assert verilog_entry["status"] == "complete"
assert verilog_entry["completed_problem_count"] == 6
print("random descriptor seed1001 verilog status check ok")
PY
```
...
- seed: 1002
- benchmark group: `RTLLM`
- run time: 2041.47 seconds
- run log:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1002/revolution/openai_gpt-oss-120b/20260619_084223_revolution_run_log.txt`
- summary:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1002/revolution/openai_gpt-oss-120b/20260619_084223_revolution_summary_results.txt`
- scheduler telemetry:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1002/revolution/openai_gpt-oss-120b/20260619_084223_revolution_scheduler_telemetry.json`
...
| --- | --- | --- |
| `Prob004_adder_8bit` | `success` | 0.38154412826809186 |
| `Prob015_multi_pipe_8bit` | `success` | 0.2232268636717548 |
| `Prob024_fsm` | `success` | 0.6834970284641851 |
| `Prob037_parallel2serial` | `success` | 0.09172757746709183 |
| `Prob041_traffic_light` | `success` | 0.39933732752360207 |
| `Prob045_alu` | `success` | 0.4193675011498785 |
| `Prob049_signal_generator` | `success` | 0.2861519004061918 |
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 15 complete, 9 pending
- arm/seed pairs: 7 standard-results complete, 1 partial, 4 pending
- `random_descriptor_qd` seed 1002: partial, 1/2 benchmark groups complete
...
payload = json.loads(p.read_text())
assert payload["manifest_summary"] == {"complete": 12, "total": 12}
assert payload["entry_summary"] == {"complete": 15, "pending": 9, "total": 24}
assert payload["arm_seed_summary"] == {
    "partial": 1,
    "pending": 4,
    "standard_results_complete": 7,
...
assert rtllm_entry["status"] == "complete"
assert rtllm_entry["completed_problem_count"] == 7
print("random descriptor seed1002 rtllm status check ok")
PY
```
...
Packaged standard results for the seed-3
`random_descriptor_qd` seed 1002 run pair.
...
UV_LINK_MODE=copy uv run --active python \
  scripts/build_auto_bd_standard_results.py \
  --run-dir exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1002/revolution/openai_gpt-oss-120b \
  --output-dir exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1002/standard_results \
  --method-name random_descriptor_qd \
  --method-family random_descriptor \
  --descriptor-version random_hash_3d \
  --phase main_screening \
  --seed 1002 \
  --run-manifest exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1002/run_manifest.json
```

Generated standard result root:

`exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1002/standard_results`
...
| --- | ---: |
| `candidate_count` | 1560 |
| `valid_ppa_candidate_count` | 696 |
| `problem_count` | 13 |
| `unique_canonical_netlist_count` | 295 |
| `unique_motif_signature_count` | 196 |
| `common_audit_bins` | 4 |
| `common_audit_total_cells` | 256 |
| `common_audit_occupied_cells` | 38 |
| `common_audit_qd_score` | 8.343239396601842 |
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 16 complete, 8 pending
- arm/seed pairs: 8 standard-results complete, 4 pending
- `random_descriptor_qd` seed 1002: standard results complete, 2/2 benchmark groups complete
...
summary_path = Path(
    "exp/auto_bd_research/main_screening_screening_seed3/"
    "random_descriptor_qd/seed_1002/standard_results/"
    "method_summary.json"
)
status = json.loads(status_path.read_text())
summary = json.loads(summary_path.read_text())
assert status["manifest_summary"] == {"complete": 12, "total": 12}
assert status["entry_summary"] == {"complete": 16, "pending": 8, "total": 24}
assert status["arm_seed_summary"] == {
    "pending": 4,
    "standard_results_complete": 8,
    "total": 12,
}
entry = next(
    item
...
assert summary["seed"] == 1002
assert summary["candidate_count"] == 1560
assert summary["valid_ppa_candidate_count"] == 696
assert summary["unique_canonical_netlist_count"] == 295
assert summary["unique_motif_signature_count"] == 196
assert summary["common_audit_occupied_cells"] == 38
print("random descriptor seed1002 standard results check ok")
PY
```

Result: `random descriptor seed1002 standard results check ok`.

## 2026-06-19 10:46 UTC

Completed the first seed-3 main-screening command for
`random_descriptor_qd` seed 1003.
...
env PYTHONPATH=src /workspace/.venv/bin/python scripts/run_backend.py \
  ...
  --post_synthesis_simulation_timeout_s 300 \
  --seed 1003 \
  --save_path /workspace/.worktrees/journal-auto-bd-exp-20260618/exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1003 \
  --search_mode revolution_qd \
  --qd_archive_type grid_quantile \
  --qd_grid_quantile_warmup_successes 8 \
  --qd_cell_mode pareto_front \
...
- seed: 1003
- benchmark group: `RTLLM`
- run time: 2005.61 seconds
- run log:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1003/revolution/openai_gpt-oss-120b/20260619_101314_revolution_run_log.txt`
- summary:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1003/revolution/openai_gpt-oss-120b/20260619_101314_revolution_summary_results.txt`
- scheduler telemetry:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1003/revolution/openai_gpt-oss-120b/20260619_101314_revolution_scheduler_telemetry.json`
...
| --- | --- | --- |
| `Prob004_adder_8bit` | `success` | 0.38154412826809186 |
| `Prob015_multi_pipe_8bit` | `success` | 0.2412308124944801 |
| `Prob024_fsm` | `success` | 0.6834970284641851 |
| `Prob037_parallel2serial` | `success` | 0.00886109067124523 |
| `Prob041_traffic_light` | `success` | 0.4736310820624546 |
| `Prob045_alu` | `success` | 0.40609333727577374 |
| `Prob049_signal_generator` | `success` | 0.25984938304829974 |
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 17 complete, 7 pending
- arm/seed pairs: 8 standard-results complete, 1 partial, 3 pending
- `random_descriptor_qd` seed 1003: partial, 1/2 benchmark groups complete
...
payload = json.loads(p.read_text())
assert payload["manifest_summary"] == {"complete": 12, "total": 12}
assert payload["entry_summary"] == {"complete": 17, "pending": 7, "total": 24}
assert payload["arm_seed_summary"] == {
    "partial": 1,
    "pending": 3,
    "standard_results_complete": 8,
...
assert rtllm_entry["status"] == "complete"
assert rtllm_entry["completed_problem_count"] == 7
print("random descriptor seed1003 rtllm status check ok")
PY
```

Result: `random descriptor seed1003 rtllm status check ok`.

## 2026-06-19 11:30 UTC
...
- seed: 1003
- benchmark group: `VerilogEval-Spec-to-RTL`
- run time: 2506.08 seconds
- run log:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1003/revolution/openai_gpt-oss-120b/20260619_104859_revolution_run_log.txt`
- summary:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1003/revolution/openai_gpt-oss-120b/20260619_104859_revolution_summary_results.txt`
- scheduler telemetry:
  `exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1003/revolution/openai_gpt-oss-120b/20260619_104859_revolution_scheduler_telemetry.json`
...
| `Prob135_m2014_q6b` | `success` | 0.33104633781763826 |
| `Prob150_review2015_fsmonehot` | `success` | 0.32968627450980387 |
| `Prob151_review2015_fsm` | `success` | -0.12350736511810335 |
| `Prob153_gshare` | `success` | 0.1336830758174632 |
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 18 complete, 6 pending
- arm/seed pairs: 8 standard-results complete, 1 runs complete, 3 pending
- `random_descriptor_qd` seed 1003: runs complete, 2/2 benchmark groups complete
...
payload = json.loads(p.read_text())
assert payload["manifest_summary"] == {"complete": 12, "total": 12}
assert payload["entry_summary"] == {"complete": 18, "pending": 6, "total": 24}
assert payload["arm_seed_summary"] == {
    "pending": 3,
    "runs_complete": 1,
    "standard_results_complete": 8,
...
assert verilog_entry["status"] == "complete"
assert verilog_entry["completed_problem_count"] == 6
print("random descriptor seed1003 verilog status check ok")
PY
```

Result: `random descriptor seed1003 verilog status check ok`.

## 2026-06-19 11:33 UTC

Packaged standard results for the seed-3
`random_descriptor_qd` seed 1003 run pair.
...
UV_LINK_MODE=copy uv run --active python \
  scripts/build_auto_bd_standard_results.py \
  --run-dir exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1003/revolution/openai_gpt-oss-120b \
  --output-dir exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1003/standard_results \
  --method-name random_descriptor_qd \
  --method-family random_descriptor \
  --descriptor-version random_hash_3d \
  --phase main_screening \
  --seed 1003 \
  --run-manifest exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1003/run_manifest.json
```

Generated standard result root:

`exp/auto_bd_research/main_screening_screening_seed3/random_descriptor_qd/seed_1003/standard_results`
...
| --- | ---: |
| `candidate_count` | 1560 |
| `valid_ppa_candidate_count` | 635 |
| `problem_count` | 13 |
| `unique_canonical_netlist_count` | 315 |
| `unique_motif_signature_count` | 213 |
| `common_audit_bins` | 4 |
| `common_audit_total_cells` | 256 |
| `common_audit_occupied_cells` | 37 |
| `common_audit_qd_score` | 7.648639574787505 |
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 18 complete, 6 pending
- arm/seed pairs: 9 standard-results complete, 3 pending
- `random_descriptor_qd` seed 1003: standard results complete, 2/2 benchmark groups complete
...
summary_path = Path(
    "exp/auto_bd_research/main_screening_screening_seed3/"
    "random_descriptor_qd/seed_1003/standard_results/"
    "method_summary.json"
)
status = json.loads(status_path.read_text())
summary = json.loads(summary_path.read_text())
assert status["manifest_summary"] == {"complete": 12, "total": 12}
assert status["entry_summary"] == {"complete": 18, "pending": 6, "total": 24}
assert status["arm_seed_summary"] == {
    "pending": 3,
    "standard_results_complete": 9,
    "total": 12,
}
entry = next(
    item
...
assert summary["seed"] == 1003
assert summary["candidate_count"] == 1560
assert summary["valid_ppa_candidate_count"] == 635
assert summary["unique_canonical_netlist_count"] == 315
assert summary["unique_motif_signature_count"] == 213
assert summary["common_audit_occupied_cells"] == 37
print("random descriptor seed1003 standard results check ok")
PY
```

Result: `random descriptor seed1003 standard results check ok`.

## 2026-06-19 12:13 UTC

Completed the first seed-3 main-screening command for
`synthesis_trajectory_nod` seed 1001.
...
env PYTHONPATH=src /workspace/.venv/bin/python scripts/run_backend.py \
  ...
  --post_synthesis_simulation_timeout_s 300 \
  --seed 1001 \
  --save_path /workspace/.worktrees/journal-auto-bd-exp-20260618/exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1001 \
  --search_mode revolution_qd \
  --qd_archive_type grid_quantile \
  --qd_grid_quantile_warmup_successes 8 \
  --qd_cell_mode pareto_front \
...
- seed: 1001
- benchmark group: `RTLLM`
- run time: 2136.22 seconds
- run log:
  `exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1001/revolution/openai_gpt-oss-120b/20260619_113754_revolution_run_log.txt`
- summary:
  `exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1001/revolution/openai_gpt-oss-120b/20260619_113754_revolution_summary_results.txt`
- scheduler telemetry:
  `exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1001/revolution/openai_gpt-oss-120b/20260619_113754_revolution_scheduler_telemetry.json`
...
| --- | --- | --- |
| `Prob004_adder_8bit` | `success` | 0.38154412826809186 |
| `Prob015_multi_pipe_8bit` | `success` | 0.08530674695865637 |
| `Prob024_fsm` | `success` | 0.6834970284641851 |
| `Prob037_parallel2serial` | `success` | 0.468610906712452 |
| `Prob041_traffic_light` | `success` | 0.4371229726458485 |
| `Prob045_alu` | `success` | 0.4140619620211577 |
| `Prob049_signal_generator` | `success` | 0.26025712884096003 |
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 19 complete, 5 pending
- arm/seed pairs: 9 standard-results complete, 1 partial, 2 pending
- `synthesis_trajectory_nod` seed 1001: partial, 1/2 benchmark groups complete
...
stage_root = Path(
    "exp/auto_bd_research/main_screening_screening_seed3/"
    "synthesis_trajectory_nod/seed_1001/revolution/"
    "openai_gpt-oss-120b/RTLLM"
)
status = json.loads(status_path.read_text())
assert status["manifest_summary"] == {"complete": 12, "total": 12}
assert status["entry_summary"] == {"complete": 19, "pending": 5, "total": 24}
assert status["arm_seed_summary"] == {
    "partial": 1,
    "pending": 2,
    "standard_results_complete": 9,
...
print("stnod seed1001 rtllm status check ok")
PY
```

Result: `stnod seed1001 rtllm status check ok`.

## 2026-06-19 13:00 UTC
...
- seed: 1001
- benchmark group: `VerilogEval-Spec-to-RTL`
- run time: 2621.81 seconds
- run log:
  `exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1001/revolution/openai_gpt-oss-120b/20260619_121615_revolution_run_log.txt`
- summary:
  `exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1001/revolution/openai_gpt-oss-120b/20260619_121615_revolution_summary_results.txt`
- scheduler telemetry:
  `exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1001/revolution/openai_gpt-oss-120b/20260619_121615_revolution_scheduler_telemetry.json`
- final ST-NOD `07_buffered.v` snapshots observed: 317
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 20 complete, 4 pending
- arm/seed pairs: 9 standard-results complete, 1 runs complete, 2 pending
- `synthesis_trajectory_nod` seed 1001: runs complete, 2/2 benchmark groups complete
...
status = json.loads(status_path.read_text())
assert status["manifest_summary"] == {"complete": 12, "total": 12}
assert status["entry_summary"] == {"complete": 20, "pending": 4, "total": 24}
assert status["arm_seed_summary"] == {
    "pending": 2,
    "runs_complete": 1,
    "standard_results_complete": 9,
...
assert verilog_entry["status"] == "complete"
assert verilog_entry["completed_problem_count"] == 6
assert len(list(stage_root.rglob("*.stnod.stages/07_buffered.v"))) == 317
print("stnod seed1001 verilog status check ok")
PY
```

Result: `stnod seed1001 verilog status check ok`.

## 2026-06-19 13:03 UTC

Packaged standard results for the seed-3
`synthesis_trajectory_nod` seed 1001 run pair.
...
UV_LINK_MODE=copy uv run --active python \
  scripts/build_auto_bd_standard_results.py \
  --run-dir exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1001/revolution/openai_gpt-oss-120b \
  --output-dir exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1001/standard_results \
  --method-name synthesis_trajectory_nod \
  --method-family synthesis_trajectory_nod \
  --descriptor-version stnod_trajectory_5d \
  --phase main_screening \
  --seed 1001 \
  --run-manifest exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1001/run_manifest.json
```

Generated standard result root:

`exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1001/standard_results`
...
| --- | ---: |
| `candidate_count` | 1560 |
| `valid_ppa_candidate_count` | 652 |
| `problem_count` | 13 |
| `unique_canonical_netlist_count` | 280 |
| `unique_motif_signature_count` | 200 |
| `common_audit_bins` | 4 |
| `common_audit_total_cells` | 256 |
| `common_audit_occupied_cells` | 39 |
| `common_audit_qd_score` | 8.653995093843593 |
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 20 complete, 4 pending
- arm/seed pairs: 10 standard-results complete, 2 pending
- `synthesis_trajectory_nod` seed 1001: standard results complete, 2/2 benchmark groups complete
...
summary_path = Path(
    "exp/auto_bd_research/main_screening_screening_seed3/"
    "synthesis_trajectory_nod/seed_1001/standard_results/"
    "method_summary.json"
)
status = json.loads(status_path.read_text())
summary = json.loads(summary_path.read_text())
...
assert summary["seed"] == 1001
assert summary["candidate_count"] == 1560
assert summary["valid_ppa_candidate_count"] == 652
assert summary["unique_canonical_netlist_count"] == 280
assert summary["unique_motif_signature_count"] == 200
assert summary["common_audit_occupied_cells"] == 39
print("stnod seed1001 standard results check ok")
PY
```
...
## 2026-06-19 15:10 UTC

Completed the seed-3 main-screening RTLLM command for
`synthesis_trajectory_nod` seed 1003.
...
env PYTHONPATH=src /workspace/.venv/bin/python scripts/run_backend.py \
  ...
  --post_synthesis_simulation_timeout_s 300 \
  --seed 1003 \
  --save_path /workspace/.worktrees/journal-auto-bd-exp-20260618/exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1003 \
  --search_mode revolution_qd \
  --qd_archive_type grid_quantile \
  --qd_grid_quantile_warmup_successes 8 \
  --qd_cell_mode pareto_front \
...
- seed: 1003
- benchmark group: `RTLLM`
- run time: 2162.70 seconds
- run log:
  `exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1003/revolution/openai_gpt-oss-120b/20260619_142916_revolution_run_log.txt`
- summary:
  `exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1003/revolution/openai_gpt-oss-120b/20260619_142916_revolution_summary_results.txt`
- scheduler telemetry:
  `exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1003/revolution/openai_gpt-oss-120b/20260619_142916_revolution_scheduler_telemetry.json`
- final ST-NOD `07_buffered.v` snapshots observed: 320
...
| --- | --- | --- |
| `Prob004_adder_8bit` | `success` | 0.38154412826809186 |
| `Prob015_multi_pipe_8bit` | `success` | 0.03032362165356123 |
| `Prob024_fsm` | `success` | 0.5845271608799917 |
| `Prob037_parallel2serial` | `success` | 0.51996893140381 |
| `Prob041_traffic_light` | `success` | 0.4402977487291213 |
| `Prob045_alu` | `success` | 0.4163917143044878 |
| `Prob049_signal_generator` | `success` | 0.25984938304829974 |
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 23 complete, 1 pending
- arm/seed pairs: 11 standard-results complete, 1 partial
- `synthesis_trajectory_nod` seed 1003: partial, 1/2 benchmark groups complete
- next pending: `synthesis_trajectory_nod` seed 1003 VerilogEval
...
stage_root = Path(
    "exp/auto_bd_research/main_screening_screening_seed3/"
    "synthesis_trajectory_nod/seed_1003/revolution/"
    "openai_gpt-oss-120b/RTLLM"
)
status = json.loads(status_path.read_text())
assert status["manifest_summary"] == {"complete": 12, "total": 12}
assert status["entry_summary"] == {"complete": 23, "pending": 1, "total": 24}
assert status["arm_seed_summary"] == {
    "partial": 1,
    "standard_results_complete": 11,
    "total": 12,
...
assert rtllm_entry["status"] == "complete"
assert rtllm_entry["completed_problem_count"] == 7
assert len(list(stage_root.rglob("*.stnod.stages/07_buffered.v"))) == 320
print("stnod seed1003 rtllm status check ok")
PY
```

Result: `stnod seed1003 rtllm status check ok`.

## 2026-06-19 15:49 UTC

Completed and packaged the final seed-3 main-screening
`synthesis_trajectory_nod` seed 1003 run pair.
...
- seed: 1003
- benchmark group: `VerilogEval-Spec-to-RTL`
- run time: 2429.58 seconds
- run log:
  `exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1003/revolution/openai_gpt-oss-120b/20260619_150748_revolution_run_log.txt`
- summary:
  `exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1003/revolution/openai_gpt-oss-120b/20260619_150748_revolution_summary_results.txt`
- scheduler telemetry:
  `exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1003/revolution/openai_gpt-oss-120b/20260619_150748_revolution_scheduler_telemetry.json`
- final ST-NOD `07_buffered.v` snapshots observed: 324
...
| `Prob135_m2014_q6b` | `success` | 0.26414050822122576 |
| `Prob150_review2015_fsmonehot` | `success` | 0.32968627450980387 |
| `Prob151_review2015_fsm` | `success` | -0.12617788624500034 |
| `Prob153_gshare` | `success` | 0.13560291914358902 |
...
UV_LINK_MODE=copy uv run --active python \
  scripts/build_auto_bd_standard_results.py \
  --run-dir exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1003/revolution/openai_gpt-oss-120b \
  --output-dir exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1003/standard_results \
  --method-name synthesis_trajectory_nod \
  --method-family synthesis_trajectory_nod \
  --descriptor-version stnod_trajectory_5d \
  --phase main_screening \
  --seed 1003 \
  --run-manifest exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1003/run_manifest.json
```

Generated standard result root:

`exp/auto_bd_research/main_screening_screening_seed3/synthesis_trajectory_nod/seed_1003/standard_results`
...
| --- | ---: |
| `candidate_count` | 1560 |
| `valid_ppa_candidate_count` | 644 |
| `problem_count` | 13 |
| `unique_canonical_netlist_count` | 282 |
| `unique_motif_signature_count` | 207 |
| `common_audit_bins` | 4 |
| `common_audit_total_cells` | 256 |
| `common_audit_occupied_cells` | 41 |
| `common_audit_qd_score` | 9.513151268962522 |
...
- manifest commands: 12 complete, 0 pending
- benchmark commands: 24 complete, 0 pending
- arm/seed pairs: 12 standard-results complete, 0 pending
- `synthesis_trajectory_nod` seed 1003: standard results complete, 2/2 benchmark groups complete
...
summary_path = Path(
    "exp/auto_bd_research/main_screening_screening_seed3/"
    "synthesis_trajectory_nod/seed_1003/standard_results/"
    "method_summary.json"
)
stage_root = Path(
    "exp/auto_bd_research/main_screening_screening_seed3/"
...
summary = json.loads(summary_path.read_text())
assert status["manifest_summary"] == {"complete": 12, "total": 12}
assert status["entry_summary"] == {"complete": 24, "total": 24}
assert status["arm_seed_summary"] == {
    "standard_results_complete": 12,
    "total": 12,
}
entry = next(
    item
...
assert entry["status"] == "standard_results_complete"
assert entry["benchmark_groups_complete"] == 2
assert not [item for item in status["entry_status"] if item["status"] == "pending"]
assert summary["method_name"] == "synthesis_trajectory_nod"
assert summary["phase"] == "main_screening"
assert summary["seed"] == 1003
assert summary["candidate_count"] == 1560
assert summary["valid_ppa_candidate_count"] == 644
assert summary["unique_canonical_netlist_count"] == 282
assert summary["unique_motif_signature_count"] == 207
assert summary["common_audit_occupied_cells"] == 41
assert len(list(stage_root.rglob("*.stnod.stages/07_buffered.v"))) == 324
print("stnod seed1003 standard results check ok")
PY
```
