...
T98 runs the same stricter credit threshold with random memory. It drops to
mean HV `0.1048`, so T97 is descriptor-positive against this direct control.
...
T92 completes the runtime descriptor gate. The pooled DeepGate PCA is now
frozen as `deepgate_pool_pc0..2`, with descriptor profile
...
T94 and T95 add the matched HV evidence. T94's exact pooled DeepGate screen
is reference-complete but negative: mean HV `0.1040` versus classic `0.1406`.
T95 then keeps the same descriptor and delays archive pressure to generation
`3` with lower fill/backfill and higher champion pressure. It improves the
...
over pure T95 DeepGate on mean HV (`0.1199` versus `0.1153`), but regresses
versus the closer same-seed T83 descriptor (`0.1369`) that used RF leaf-ID,
branching, and wire-density axes. It also still loses classic mean HV
(`0.1406`), Pareto breadth (`1.625` versus `3.25`), and reference-beating
candidates (`5.375` versus `8.00`). Keep it as the hybrid pretrained
RTL/netlist representative, not a full-RTLLM spend arm.
