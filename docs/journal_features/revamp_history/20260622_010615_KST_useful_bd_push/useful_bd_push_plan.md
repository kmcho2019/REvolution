...
Current next target: execute T41, the adaptive sparse-yield gate. T41 keeps
the primary grid-quantile warmup at `8`, then falls back after generation `1`
to `4` valid PPA successes only if the archive is still empty and descriptor
geometry is ready. The first report artifact must be
`figures/t41_raw_area_power_fronts.png`, generated by
`scripts/package_t41_adaptive_sparse_yield_gate.py`, so the result is judged
from straightforward raw area-power Pareto/front geometry before archive-space
or BD-space views.
