#!/usr/bin/env python3
"""Package the MasterRTL front-slot live probe."""
...
def main() -> None:
    ...
    write_csv(tables / "masterrtl_front_slot_problem_deltas.csv", deltas)
    write_json(tables / "run_summary.json", run_summary(aggregate, deltas))
    plot_mean_hv(aggregate, figures / "mean_hv_by_backend.png")
    plot_problem_deltas(deltas, figures / "masterrtl_front_slot_hv_delta.png")
        ...
        result.append(
            {
                ...
                "front_slot_ref_beating": fnum(new["reference_beating_count"]),
...
def plot_problem_deltas(rows: list[TableRow], path: Path) -> None:
    ...
    ax.set_title("MasterRTL front-slot: per-problem HV delta")
    ax.grid(axis="y", alpha=0.25)
    ax.set_axisbelow(True)
    for index, value in enumerate(values):
        if abs(value) < 0.001:
