...
| DeepGate / DeepGate2 | Model valid, bridge partially unblocked | Official python-deepgate pretrained path works on shipped AIG examples. The latch-free bridge embeds `24` rows across `2/8` problems. The corrected transition bridge embeds `60` rows across `5/8` problems with pairwise cosine mean `0.9213`. | Fix large-design coverage and same-problem clustering before live spend. |
