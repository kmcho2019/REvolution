...
export RUN_ROOT=exp/useful_bd_push/prelim_aux_archive_seed_replication_20260625_232854_UTC/live
mkdir -p "$RUN_ROOT"
```
...
env OPENAI_API_KEY="$OPENAI_API_KEY" PYTHONPATH="$PYTHONPATH" uv run python scripts/run_backend.py \
  ...
  --save_path "$RUN_ROOT/classic_revolution_8x5/seed_$SEED"
```

## Auxiliary Archive Seeds
...
env OPENAI_API_KEY="$OPENAI_API_KEY" PYTHONPATH="$PYTHONPATH" uv run python scripts/run_backend.py \
  ...
  --save_path "$RUN_ROOT/masterrtl_aux_archive_high_exploit_8x5/seed_$SEED"
```
