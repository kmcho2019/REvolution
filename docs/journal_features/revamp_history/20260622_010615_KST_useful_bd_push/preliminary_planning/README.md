...
| `20260626_aurora_raw_impl_delayed_probe/` | Test the live AURORA-style raw implementation feature representative on the delayed high-exploit QD substrate. | Completed; screened negative versus classic, retained as the AURORA/raw-implementation category representative |
| `20260626_front_memory_replay_gate/` | Replay whether scalar top-k selection discards final PPA-front candidates that a QD memory could rescue. | Completed; classic has a real retention gap, but T96's larger gap shows broad archive recall alone is not enough |
| `20260626_fg_qdm_contribution_audit/` | Audit whether completed FG-QDM memory lanes produced valid-PPA or front-adding children before any further live spend. | Completed; exact T85/T86/T87 FG-QDM variants are negative, and T97 has now run as a stricter config-gated follow-up |
| `20260626_front_credit_rf_leafid_fg_qdm_probe/` | Test T97-style front-credit FG-QDM with the validated T83 RF leaf-ID structural descriptor axes. | Completed; T100 is the best FG-QDM smoke by mean HV, but still trails classic and is not promoted |
