...
| DeepGate / synthesized-netlist encoder | `T90_deepgate_cone_bridge_probe` | n/a | Keep as category representative; offline bridge coverage reaches `8/8` screen problems with full-transition plus cone embeddings, but no live HV exists. |
...
| DeepGate transition/cone AIG | Official pretrained bridge embeds generated candidates across all `8/8` screen problems offline when full-transition rows are combined with T90 cone rows. | Not spend-ready: candidate-level pooling and a replay/live HV screen are still missing. |
