...
METHOD_NAME="${METHOD_NAME:-pcn_sr_quality_memory_8x5}"

uv run python scripts/run_backend.py \
  --backend revolution \
  --benchmarks RTLLM \
  --problems "${PCN_PROBLEMS[@]}" \
  --api_backend vllm \
  --vllm_host "$VLLM_HOST" \
  --vllm_port "$VLLM_PORT" \
  ...
  --repair_kind none \
  --qd_descriptor_axes sr_pca_0 sr_pca_1 sr_pca_2 \
  --qd_descriptor_file "$SR_RAW_PCA_DESCRIPTOR_FILE" \
  --save_path "$(method_save_path "$METHOD_NAME")"
