...
| `L4` | Learned encoders | Test Qwen3, DeepGate, graph, sequence, AURORA, and multimodal circuit embeddings. | T44 gives a live top-8 graph signal but fails the yield gate; next try fewer or compressed T11 runtime axes. |
...
| T44 | `L4/L5` | Live-safe top-8 subset of T11 structural graph features on the T39 one-slot sparse-warmup substrate. | Completed `T0 mixed_diagnostic`: traffic-light and multi-pipe win HV and add pooled raw-front signal, but aggregate HV/reference-beating count fall and ALU/traffic-light valid-PPA yield drops exceed the gate. | `ablate` dimensionality | Pre-register a top-3/top-4 or frozen non-PPA T11 runtime projection before top-16/top-64 fitted projection. |
