...
| `L4` | Learned encoders | Test Qwen3, DeepGate, graph, sequence, AURORA, and multimodal circuit embeddings. | T37 confirms the T11-family replay lead is exactly one bounded local-front slot; T39 now tests the sparse-yield live warmup repair. |
| `L5` | Archive coupling and parent pressure | Preserve diversity while restoring hill-climbing pressure. | T39 is the current live archive-coupling follow-up for champion-plus-one-slot retention under sparse yield. |
...
| T39 | `L5` | Sparse-yield warmup ablation of T38: keep the one-slot rule and lower grid-quantile warmup from 8 to 4. | Pre-registered; no live result yet. | `ablate` | Run the bounded arm and package direct PPA-front figures before any broader controls. |
