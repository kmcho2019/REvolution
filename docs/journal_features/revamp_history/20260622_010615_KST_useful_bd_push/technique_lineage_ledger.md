...
| `L4` | Learned encoders | Test Qwen3, DeepGate, graph, sequence, AURORA, and multimodal circuit embeddings. | T37 confirms the T11-family replay lead is exactly one bounded local-front slot; T42 is the current live timing ablation before exact T11 projection. |
| `L5` | Archive coupling and parent pressure | Preserve diversity while restoring hill-climbing pressure. | T42 is pre-registered to test generation-0 sparse-yield gating after T41's late multi-pipe failure. |
