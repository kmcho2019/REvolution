...
| `L5` | Archive coupling and parent pressure | Preserve diversity while restoring hill-climbing pressure. | T50 is pre-registered as the budget/front follow-up after T49. |
| `L6` | Lineage and emitter schedules | Bias exploration with repair dynamics, parent history, and adaptive emitters. | T50 separates visible candidate-budget matching from T49's repair-only role separation. |
...
| T50 | `L5/L6` | Candidate-matched thought-only front retention: restore the 12-candidate evaluated-code budget, disable repair, and widen per-cell Pareto retention. | Pre-registered, no live result yet. | `advance` candidate/front control | Run seed `1001` against T47 classic roots before seed `1002` or held-out spend. |
