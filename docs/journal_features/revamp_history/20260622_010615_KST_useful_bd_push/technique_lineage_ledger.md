...
| `L7` | RTL-native descriptors | Use RTL operator graphs and timing-risk/path morphology as behavior axes. | T74 is the registered shape-density plus near-front-gated single-thought pairing follow-up after T73's yield-positive, HV/front-breadth-negative result. |
...
| T74 | `L7/L5` | T73 shape-density cells with `near_front_descriptor` gating for the existing low-rate single-thought two-parent prompt requests. | Pre-registered; no live result yet. | `run` bounded hard/tuning screen | Must preserve classic-covered problems, stay near-classic or better on headline front/HV metrics, and improve the T51/T66/T67/T72/T73 lineage without yield-only overclaiming. |
