...
| `L4` | Learned encoders | Test Qwen3, DeepGate, graph, sequence, AURORA, and multimodal circuit embeddings. | T45 is pre-registered to test compact top-4 T11 runtime graph axes after T44's yield-gated top-8 signal. |
...
| T44 | `L4/L5` | Live-safe top-8 subset of T11 structural graph features on the T39 one-slot sparse-warmup substrate. | Completed `T0 mixed_diagnostic`: traffic-light and multi-pipe win HV and add pooled raw-front signal, but aggregate HV/reference-beating count fall and ALU/traffic-light valid-PPA yield drops exceed the gate. | `ablate` dimensionality | T45 pre-registers the compact top-4 follow-up. |
| T45 | `L4/L5` | Compact top-4 T11 runtime graph profile on the same T39/T44 one-slot sparse-warmup substrate. | Pre-registered; live result pending. | `advance` live screen | Run the matched three-problem live screen, direct raw-PPA package, and full Phase 03.1 viewer before any tier decision. |
