...
| T75 | `L7/L5` | T73 shape-density cells with explicit `qd_front_slot_lane_fraction=0.30` and one-parent-only single-thought prompts. | Pre-registered after adding the configurable front-slot pressure knob. No result claim yet. | `advance` live screen | Run seed `1001` only after storage/vLLM preflight; compare against classic, T73, and T74 on reference-complete front metrics. |
