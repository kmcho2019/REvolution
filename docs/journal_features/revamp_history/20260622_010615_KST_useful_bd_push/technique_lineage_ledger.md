...
| `L7` | RTL-native descriptors | Use RTL operator graphs and timing-risk/path morphology as behavior axes. | T66 is pre-registered to couple RTL-native state/pipeline cells to parent choice. |
...
| T66 | `L7` | T63 state/pipeline RTL-native cells used for front-slot parent pressure and low-rate near-front descriptor-compatible fusion. | Pre-registered; no result yet. | `advance` coupled parent screen | Run seed `1001` only after descriptor probe and vLLM preflight; compare with classic, T51, T63, and T64. |
