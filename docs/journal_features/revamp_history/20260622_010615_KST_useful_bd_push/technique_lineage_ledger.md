...
| `L5` | Archive coupling and parent pressure | Preserve diversity while restoring hill-climbing pressure. | T85 proves front-guarded memory can run but loses the smoke; T86 shows SR memory does not beat random memory, so exact SR-memory FG-QDM is retired. |
...
| T12/T18 | `L6` | Lineage repair and adaptive emitter scheduling. | T18 is retired by existing evidence: T57's adaptive rebinning never actuates, and T32's front-preserving emitter does not preserve T26's holdout quality/HV signal. T12 remains a scaffolded lineage-repair follow-up. | `hybridize` only with a materially different mechanism | Convert the yield/front gap into source-level repair or front-rescue emitters with explicit valid-PPA and front-add counters. |
