...
| T82 | `L7` | Runtime descriptor hook for MasterRTL RF timing model-state metrics. | `T0_live_smoke_positive_not_screened`: evaluator smoke emits `51` timing paths, `14` unique RF leaf rows, and `161` unique RF leaf IDs; one live smoke emits one archive member with `17` unique RF leaf rows and `319` unique leaf IDs. | `advance` frozen screen | Run the frozen `8x5` screen before any full RTLLM spend. |
