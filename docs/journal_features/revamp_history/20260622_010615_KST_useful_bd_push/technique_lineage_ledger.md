...
| T67 | `L7/L6` | RTL-native state/pipeline cells plus seeded thought-code realization that refines successful parent RTL for most samples. | Pre-registered; no result yet. | `advance` seed `1001` | Test source-preserving generation before adding repair selectors or another parent-pressure tweak. |
