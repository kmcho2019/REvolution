...
| `L7` | RTL-native descriptors | Use RTL operator graphs and timing-risk/path morphology as behavior axes. | T63 is the pre-registered live screen for fused Yosys-SOG and timing-risk axes; no live claim exists yet. |
...
| T15/T60/T61/T62/T63 | `L7` | Yosys-SOG/MasterRTL and RTLTimer timing-risk/path-morphology descriptors. | T63 is pre-registered as the live screen after T62's `T0 positive_proxy_not_promoted` fused proxy clue. | `advance` seed-1001 live screen | Treat these as behavior-descriptor lanes, not direct PPA predictors; pair the live test with T51/T26-family archive machinery and reference-complete PPA claims. |
