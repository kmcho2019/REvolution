...
| T83 | `L7/L8` | RF leaf-ID structural delayed QD: use RF timing leaf-ID breadth as a secondary coordinate beside MasterRTL branching and RTLTimer wire density, with delayed archive activation. | `pre_registered_not_run`: endpoint preflight passed and the frozen eight-design command is documented. | `advance` frozen screen | Run the frozen `8x5` screen and decide before any full RTLLM spend. |
