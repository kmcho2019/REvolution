...
| `L7` | RTL-native descriptors | Use RTL operator graphs and timing-risk/path morphology as behavior axes. | T72 pre-registers source-aligned cell coupling; implement the exact runtime hook before live spend. |
...
| T72 | `L7` | Source-aligned RTL-cell QD method card using T71 cells inside T66-style front-slot parent pressure. | Pre-registered; no live result. Execution is blocked until `source_aligned_masterrtl_rtltimer_cell_2d` is implemented and a runtime probe reproduces T71 cells without PPA leakage. | `advance` runtime hook | Implement the descriptor hook and T71 regression probe before vLLM spend. |
