...
| `L4` learned encoders | Try Qwen, DeepGate, DeepSeq, NetTAG, CircuitFusion, MGVGA, DE-HNN, DeepCell, AURORA. | T33 is `T0`: RTL views modestly beat lexical HV, but netlist collapse fixes do not improve PPA-front metrics. | Run T34 bounded PCA-residual replay, then move to graph encoders if it fails. |
...
| `L4` | T33/T34 Qwen projection ladder | T33 completed `T0 diagnostic`; T34 is registered. | T33 signals do not align on direct PPA-front metrics. | Run T34 label-free PCA residuals before any heavier Qwen head. |
...
Current follow-up: T34 is the bounded Qwen step. If label-free PCA residuals
do not improve both replay utility and collapse diagnostics, move the lane to
graph/multimodal encoders instead of adding more Qwen whole-design variants.
...
| `L4` learned encoders | T06-T16, T33, T34 | Candidate split branch. | Run T34 PCA-residual replay or move to graph encoders. | Learned features improve PPA-front/HV metrics without problem-ID collapse. |
