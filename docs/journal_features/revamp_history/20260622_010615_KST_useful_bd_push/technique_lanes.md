...
| `L8` budget and benchmark shape | Is the current evaluation structure hiding QD benefits? | Fixed-total-budget shape ablation, discriminative medium-validity design sets. | QD improves more than classic when archive cells have enough depth to mature. | Deeper runs help both methods equally, or chosen designs are saturated, invalid-heavy, or missing reference PPA. |
...
| `L7` RTL-native descriptors | Use pre-synthesis RTL structure and timing-risk morphology as behavior axes. | T72 runs the source-aligned T71 cells live and lands near classic on mean HV; T73 improves valid-PPA yield and archive occupancy but loses classic on HV/Pareto breadth; T74 preserves coverage but regresses on HV/yield. | Package T75, then open a verification-gated MasterRTL pretrained tree-leaf embedding lane if T75 does not close the front gap. |
| `L8` budget and benchmark shape | Test whether wide/shallow budgets and saturated tasks hide QD value. | Current `12 x 3` screens likely favor classic, but this is a hypothesis rather than a proven result. | After T75 packaging, run a fixed-total-budget ablation on a frozen medium-validity, reference-complete subset. |
...
| `L7` | T15 Yosys-SOG, T60/T61 RTLTimer timing-risk, T62 fused descriptors, T63 live screen, T64 operator/timing ablation, T65 secondary-cell audit, T66 coupled parent method, T67 seeded thought-code method, T68 upstream verification, T69 open-Yosys preprocessing, T70 generated RTL smoke, T71 source-aligned feature map, T72 source-aligned cell QD, T73 shape-density QD, T74 shape-density front-slot hybrid, and T75 shape-density front-pressure QD | T72 is measured `T1 near_classic_not_promoted`; T73 is `T0 positive_diagnostic_not_promoted`; T74 is `T0 diagnostic_regression_not_promoted`; T75 is pending packaging. | Exact T72 cells are interpretable but too collapsed; T73 cells improve yield but do not create enough front material; T74's rare gated pairs do not fix that. | Package T75, then verify MasterRTL pretrained inference before any tree-leaf or prediction-risk BD claim. |
...
| `L7` RTL-native descriptors | T15, T60, T61, T62, T63, T64, T65, T66, T67, T68, T69, T70, T71, T72, T73, T74, T75 | T74 completed as a regression after T73's yield-positive but front-negative result. | Run T75 as the direct front-pressure follow-up. | A successor improves live front metrics without default-reference or PPA-leakage claims. |
