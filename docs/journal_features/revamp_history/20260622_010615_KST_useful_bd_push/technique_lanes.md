...
| `L7` RTL-native descriptors | Use pre-synthesis RTL structure and timing-risk morphology as behavior axes. | T60 now has a first `T0 diagnostic_proxy` audit: interpretable timing-risk geometry, but no front-cell advantage over classic. T15 MasterRTL SOG remains scaffolded. | Replace the regex timing-risk proxy with true RTLTimer or MasterRTL/SOG extraction, then pair problem-local cells with T51/T26-family archive machinery if collapse checks pass. |
...
| `L7` | T15 MasterRTL SOG and T60 RTLTimer timing-risk descriptors | Active diagnostic lane after the T26/T59 corrections. | T60 proxy is not positive: pooled coverage ties classic, and problem-balanced T26 occupied-cell deltas are negative. | Escalate to true RTLTimer/MasterRTL preprocessing before any live claim; use reference-complete paired claims and avoid treating timing prediction as the BD objective. |
