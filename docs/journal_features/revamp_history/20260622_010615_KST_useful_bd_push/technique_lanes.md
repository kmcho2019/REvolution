...
| `L5` archive coupling | Preserve hill-climbing pressure without collapsing to scalar weighted-sum fitness. | Auxiliary archive, delayed activation, stagnation activation, and FG-QDM through T97 all ran; the closest replicated and smoke results remain negative versus classic. | Stop small front-slot/fill/schedule tweaks. The next coupling method must be materially different and must prove memory-lane contribution per LLM call. |
...
| `L5` | T17/T23-T59 local-Pareto lineage, auxiliary archive probes, delayed/stagnation activation, and T85-T87/T97 FG-QDM | T97 is the best FG-QDM smoke by mean HV, but guarded memory still trails classic and has no memory-lane global-front adds; auxiliary archive replication is also negative. | Retire small schedule/front-slot/fill tweaks. | A candidate must improve front material or memory-lane front-add rate per LLM call without hidden duplicate loss or default-reference headline dependence. |
...
| 2026-06-26 | `L5` archive coupling | T97 front-credit FG-QDM smoke | `park` best FG-QDM smoke | T97 raises FG-QDM smoke mean HV to `0.1534`, above earlier SR, random, and shape-density FG-QDM smokes, but still trails classic `0.1903` and has `0` memory-lane global-front additions. | Keep T97 as the FG-QDM category representative. Do not widen until a same-threshold random control or a memory-refine diagnostic shows global-front contribution. |
...
delayed/stagnation archive activation, and T85-T87/T97 FG-QDM are all measured
...
| `L5` archive coupling | T17, T23-T59, auxiliary archive probes, delayed/stagnation activation, T85-T87, T97 | Active but negative: auxiliary archive replication, delayed/stagnation activation, and FG-QDM all trail classic. | Do not run another small fill/front-slot/schedule tweak. | A candidate improves front material or memory-lane front-add rate per LLM call without hidden duplicate loss or default-reference headline dependence. |
