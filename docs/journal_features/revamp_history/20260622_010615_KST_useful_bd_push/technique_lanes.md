...
| `L5` archive coupling | Preserve hill-climbing pressure without collapsing to scalar weighted-sum fitness. | Auxiliary archive, delayed activation, stagnation activation, and FG-QDM through T98 all ran; the closest replicated and smoke results remain negative versus classic. | Stop small front-slot/fill/schedule tweaks. The next coupling method must be materially different and must prove memory-lane contribution per LLM call. |
...
| 2026-06-26 | `L5` archive coupling | T98 front-credit random-memory control | `park` FG-QDM family | T98 falls to mean HV `0.1048`, so T97 beats the direct random control. T98 has one memory-refine global-front add, but it is not final-front productive and classic still dominates. | Keep T97 as category representative only. Do not widen FG-QDM without a mechanism that adds quality-productive global-front material. |
...
delayed/stagnation archive activation, and T85-T87/T97/T98 FG-QDM are all measured
...
| `L5` archive coupling | T17, T23-T59, auxiliary archive probes, delayed/stagnation activation, T85-T87, T97, T98 | Active but negative: auxiliary archive replication, delayed/stagnation activation, and FG-QDM all trail classic. | Do not run another small fill/front-slot/schedule tweak. | A candidate improves front material or memory-lane front-add rate per LLM call without hidden duplicate loss or default-reference headline dependence. |
