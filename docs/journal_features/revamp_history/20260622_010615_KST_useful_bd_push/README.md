...
- First completed method package: `techniques/T01_simple_yosys_stat_bd/`
  (`T0 diagnostic`).
- Most recent package:
  `techniques/T29_sr_raw_front_recovery_qd/` (completed front-recovery
  parent-source ablation; `T0 diagnostic` because it loses multi-pipe final-PPA
  coverage and does not recover the front deficit).
- Next registered package:
  `techniques/T30_t26_holdout_front_audit/` (classic versus T26 conservative
  exploit on the frozen VerilogEval holdout screen).
- Next direction after T30:
  if T26 fails holdout, specify a repair/yield/front-preserving emitter; do
  not keep interpolating SR raw/T26 scheduler knobs blindly.
...
| `T30` | `T30_t26_holdout_front_audit` | Pre-registered holdout audit; compares classic versus exact T26 conservative-exploit SR raw on the frozen VerilogEval holdout screen. |
