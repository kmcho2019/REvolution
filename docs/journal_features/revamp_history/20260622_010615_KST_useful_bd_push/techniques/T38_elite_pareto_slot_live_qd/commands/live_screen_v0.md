# T38 Live Screen V0 Command

Run preflight first:

```bash
RUN_TS="$(date -u +%Y%m%d_%H%M%S_UTC)"
RUN_ROOT="exp/useful_bd_push/t38_elite_pareto_slot_live_qd_${RUN_TS}"
...
Full local Pareto-cell control:
...
uv run python scripts/run_backend.py \
  ...
  --seed 1001 \
  --save_path "${RUN_ROOT}/graph_pareto_front_qd/seed_1001" \
  --no-backend_subdir
```

T38 bounded one-slot arm:
...
uv run python scripts/run_backend.py \
  ...
  --qd_descriptor_file data/configs/qd_descriptor_profiles.yaml \
  --seed 1001 \
  --save_path "${RUN_ROOT}/elite_pareto_slot_qd/seed_1001" \
  --no-backend_subdir
```

Validate the T38 bounded arm:

```bash
uv run python scripts/validate_pareto_front_run.py \
  --run-root "${RUN_ROOT}" \
  --subset-config docs/journal_features/revamp_history/20260622_010615_KST_useful_bd_push/techniques/T38_elite_pareto_slot_live_qd/tables/live_screen_v0_subset.yaml \
  --classic-mode classic_revolution \
  --pareto-qd-mode elite_pareto_slot_qd \
  --require-full-subset
```
