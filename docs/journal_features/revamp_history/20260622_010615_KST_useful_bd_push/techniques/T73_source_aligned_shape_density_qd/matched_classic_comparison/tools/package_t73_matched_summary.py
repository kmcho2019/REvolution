#!/usr/bin/env python3
"""Package compact T73 matched-comparison tables and figures."""
...
TECHNIQUE_ROOT = ROOT.parents[0]
REVAMP_ROOT = TECHNIQUE_ROOT.parents[1]
ANALYSIS = Path("exp/useful_bd_push/t73_matched_classic_comparison_20260624_001300_UTC/final_analysis")
TABLES = ROOT / "tables"
DATA = ROOT / "data"
...
def write_completeness_table() -> None:
    ...
    live_rows = {
        (row["suite"], row["problem"]): row
        for row in read_rows(TECHNIQUE_ROOT / "tables" / "t73_live_screen_status.csv")
    }
    ...
    for benchmark, problem in sorted(refs):
        ...
        qd = counts.get((benchmark, problem, T73), 0)
        status = "headline"
        if classic == 0 or qd == 0:
            status = "diagnostic_only"
        live = live_rows[(benchmark, problem)]
        rows.append(
            {
                "benchmark": benchmark,
                "problem": problem,
                ...
                "qd_live_summary_status": live["summary_status"],
                "qd_archive_members": live["archive_members"],
                "qd_occupied_cells": live["occupied_cells"],
            }
        )
    write_rows(TABLES / "t73_ppa_completeness.csv", rows)
...
def plot_hv_delta() -> None:
    ...
    for key in sorted(by_problem):
        pair = by_problem[key]
        labels.append(problem_label(pair[T73]))
        deltas.append(float(pair[T73]["hypervolume"]) - float(pair[CLASSIC]["hypervolume"]))
    ...
    axis.set_title("T73 minus classic hypervolume by problem", fontsize=15, pad=12)
    ...
    fig.tight_layout()
    fig.savefig(FIGURES / "t73_hv_delta_by_problem.png", dpi=180)
    plt.close(fig)


def plot_valid_ppa_counts() -> None:
    rows = read_rows(TABLES / "t73_ppa_completeness.csv")
    labels = [problem_label(row) for row in rows]
    classic = [int(row["classic_valid_ppa_count"]) for row in rows]
    t73 = [int(row["qd_valid_ppa_count"]) for row in rows]
    xs = list(range(len(labels)))

    fig, axis = plt.subplots(figsize=(11.5, 4.8))
    axis.bar([x - 0.18 for x in xs], classic, width=0.36, label="classic", color="#2f79b7")
    axis.bar([x + 0.18 for x in xs], t73, width=0.36, label="T73", color="#d87932")
    ...
    axis.spines[["top", "right"]].set_visible(False)
    fig.tight_layout()
    fig.savefig(FIGURES / "t73_valid_ppa_counts.png", dpi=180)
    plt.close(fig)
...
def copy_representative_fronts() -> None:
    ...
    for source, name in {
        base / "RTLLM" / "Prob041_traffic_light" / "gain" / "power_vs_area.png": "rtllm_prob041_traffic_gain_power_area.png",
