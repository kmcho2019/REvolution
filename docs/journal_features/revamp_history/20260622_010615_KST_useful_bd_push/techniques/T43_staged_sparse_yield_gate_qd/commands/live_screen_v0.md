# T43 Live Screen V0 Commands

Run preflight first:

```bash
RUN_TS="$(date -u +%Y%m%d_%H%M%S_UTC)"
RUN_ROOT="exp/useful_bd_push/t43_staged_sparse_yield_gate_qd_${RUN_TS}"
...
uv run python scripts/run_backend.py \
  ...
  --qd_adaptive_warmup_champion_lane_fraction 0.60 \
  ...
  --representation_kind code_individual \
  --save_path "${RUN_ROOT}/staged_sparse_yield_gate_qd/seed_1001"
```
...
uv run python scripts/validate_pareto_front_run.py \
  ...
  --pareto-qd-mode staged_sparse_yield_gate_qd \
  --require-full-subset
```
...
```bash
uv run python scripts/package_t43_staged_sparse_yield_gate.py \
  --t43-run-root "${RUN_ROOT}" \
  --t42-run-root exp/useful_bd_push/t42_initial_sparse_yield_gate_qd_20260622_093940_UTC \
  ...
  --output-dir docs/journal_features/revamp_history/20260622_010615_KST_useful_bd_push/techniques/T43_staged_sparse_yield_gate_qd
```
