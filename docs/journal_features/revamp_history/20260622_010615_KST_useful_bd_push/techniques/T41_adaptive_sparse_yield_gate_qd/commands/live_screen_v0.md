# T41 Live Screen V0 Commands

Run preflight first:

```bash
RUN_TS="$(date -u +%Y%m%d_%H%M%S_UTC)"
RUN_ROOT="exp/useful_bd_push/t41_adaptive_sparse_yield_gate_qd_${RUN_TS}"
...
uv run python scripts/run_backend.py \
  ...
  --representation_kind code_individual \
  --save_path "${RUN_ROOT}/adaptive_sparse_yield_gate_qd/seed_1001"
```
...
uv run python scripts/validate_pareto_front_run.py \
  --run-root "${RUN_ROOT}" \
  --subset-config docs/journal_features/revamp_history/20260622_010615_KST_useful_bd_push/techniques/T41_adaptive_sparse_yield_gate_qd/tables/live_screen_v0_subset.yaml \
  --classic-mode classic_revolution \
  --pareto-qd-mode adaptive_sparse_yield_gate_qd \
  --require-full-subset
```
...
```bash
uv run python scripts/package_t41_adaptive_sparse_yield_gate.py \
  --t41-run-root "${RUN_ROOT}" \
  ...
  --output-dir docs/journal_features/revamp_history/20260622_010615_KST_useful_bd_push/techniques/T41_adaptive_sparse_yield_gate_qd
```

The primary generated figure is
`figures/t41_raw_area_power_fronts.png`. It must be inspected before the run
is interpreted. Use raw area on x, raw power on y, no inverted axes, and
lower-left as better.

The T41 report should compare against the matched T41 classic arm, frozen T40
manual/random/full-Pareto controls, and the frozen T39 one-slot reference.
