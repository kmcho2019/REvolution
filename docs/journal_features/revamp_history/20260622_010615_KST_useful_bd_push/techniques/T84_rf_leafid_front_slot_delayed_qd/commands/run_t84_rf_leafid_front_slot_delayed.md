# RF Leaf-ID Front-Slot Delayed Command

```bash
RUN_ROOT=exp/useful_bd_push/prelim_rf_leafid_front_slot_delayed_20260626_062400_UTC/live
OPENAI_API_KEY="${OPENAI_API_KEY:-vllm-local-placeholder}" PYTHONPATH=src uv run python scripts/run_backend.py \
  --backend revolution \
  ...
  --save_path "$RUN_ROOT/masterrtl_rf_leafid_front_slot_delayed_8x5/seed_1001"
```
