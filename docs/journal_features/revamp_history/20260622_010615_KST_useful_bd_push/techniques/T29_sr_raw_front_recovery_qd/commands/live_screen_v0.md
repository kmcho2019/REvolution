# T29 Live Screen V0 Command

Run preflight first:

```bash
RUN_TS="$(date -u +%Y%m%d_%H%M%S_UTC)"
RUN_ROOT="exp/useful_bd_push/t29_sr_raw_front_recovery_qd_${RUN_TS}"
...
/workspace/.venv/bin/python scripts/run_backend.py \
  ...
  --qd_objectives ppa \
  --qd_champion_lane_fraction 0.60 \
  --qd_parent_selection nsga2_global_rank \
  --qd_two_parent_probability 0.20 \
  ...
  --seed 1001 \
  --save_path "${RUN_ROOT}/sr_raw_front_recovery_qd/seed_1001" \
  --no-backend_subdir \
  2>&1 | tee "${RUN_ROOT}/logs/sr_raw_front_recovery_qd_seed_1001.log"
```
