# T51 Hard/Tuning Sanity Commands
...
RUN_ROOT="exp/useful_bd_push/t51_code_thought_front_slot_${RUN_TS}/hard_tuning"
...
uv run python scripts/run_backend.py \
  ...
  --save_path "${RUN_ROOT}/code_thought_front_slot_qd/seed_${SEED}"
```

## Validators

Run after the QD arm exits. These commands validate T51's own operator and
archive artifacts. The matched classic comparison is handled by the packaging
step below because the classic roots are reused from T47.

```bash
uv run python scripts/validate_single_thought_operator_run.py \
  --run-root "${RUN_ROOT}" \
  --subset-config docs/journal_features/revamp_history/20260622_010615_KST_useful_bd_push/techniques/T51_code_thought_front_slot_qd/tables/hard_tuning_subset.yaml \
  --classic-mode code_thought_front_slot_qd \
  --eoh-mode code_thought_front_slot_qd \
  --unified-mode code_thought_front_slot_qd \
  --require-full-subset
...
uv run python scripts/validate_pareto_front_run.py \
  ...
  --subset-config docs/journal_features/revamp_history/20260622_010615_KST_useful_bd_push/techniques/T51_code_thought_front_slot_qd/tables/hard_tuning_subset.yaml \
  --classic-mode code_thought_front_slot_qd \
  --pareto-qd-mode code_thought_front_slot_qd \
  --require-full-subset
```
...
uv run python scripts/package_t48_gated_probe.py \
  ...
  --qd-root "${RUN_ROOT}/code_thought_front_slot_qd" \
  ...
  --output-dir docs/journal_features/revamp_history/20260622_010615_KST_useful_bd_push/techniques/T51_code_thought_front_slot_qd/hard_tuning_package \
  --seed 1001 \
  --package-tag t51 \
  --package-title "T51 Code-Thought Front-Slot QD" \
  --qd-method code_thought_front_slot_qd \
  --qd-label "T51 code-thought front-slot QD" \
