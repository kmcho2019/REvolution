# T53 Hard/Tuning Sanity Commands
...
RUN_ROOT="exp/useful_bd_push/t53_sparse_front_trigger_${RUN_TS}/hard_tuning"
...
uv run python scripts/run_backend.py \
  ...
  --save_path "${RUN_ROOT}/code_thought_sparse_front_trigger_qd/seed_${SEED}"
```

## Validators

Run after the QD arm exits. These commands validate T53's own operator and
archive artifacts. The matched classic comparison is handled by packaging.
...
uv run python scripts/validate_single_thought_operator_run.py \
  ...
  --classic-mode code_thought_sparse_front_trigger_qd \
  --eoh-mode code_thought_sparse_front_trigger_qd \
  --unified-mode code_thought_sparse_front_trigger_qd \
  --require-full-subset
...
uv run python scripts/validate_pareto_front_run.py \
  ...
  --subset-config docs/journal_features/revamp_history/20260622_010615_KST_useful_bd_push/techniques/T53_sparse_front_trigger_qd/tables/hard_tuning_subset.yaml \
  --classic-mode code_thought_sparse_front_trigger_qd \
  --pareto-qd-mode code_thought_sparse_front_trigger_qd \
  --require-full-subset
```
...
uv run python scripts/package_t48_gated_probe.py \
  ...
  --qd-root "${RUN_ROOT}/code_thought_sparse_front_trigger_qd" \
  ...
  --output-dir docs/journal_features/revamp_history/20260622_010615_KST_useful_bd_push/techniques/T53_sparse_front_trigger_qd/hard_tuning_package \
  --seed 1001 \
  --package-tag t53 \
  --package-title "T53 Sparse-Front Trigger QD" \
  --qd-method code_thought_sparse_front_trigger_qd \
  --qd-label "T53 sparse-front trigger QD" \
  --counter-stem operator_counters \
  --counter-title "Operator Counters" \
  --counter-keys success_parent_requests,sparse_front_trigger_batches,sparse_front_trigger_parent_requests,two_parent_attempts
```
