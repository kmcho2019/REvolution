# T55 Hard/Tuning Sanity Commands
...
RUN_ROOT="exp/useful_bd_push/t55_coarse_sr2_front_slot_${RUN_TS}/hard_tuning"
...
uv run python scripts/run_backend.py \
  ...
  --save_path "${RUN_ROOT}/code_thought_coarse_sr2_front_slot_qd/seed_${SEED}"
```

Before accepting the live run, inspect the emitted summary/archive artifacts
and confirm `descriptor_axes` is exactly `["sr_pca_0", "sr_pca_1"]`.
...
uv run python scripts/validate_single_thought_operator_run.py \
  --run-root "${RUN_ROOT}" \
  --subset-config docs/journal_features/revamp_history/20260622_010615_KST_useful_bd_push/techniques/T55_coarse_sr2_front_slot_qd/tables/hard_tuning_subset.yaml \
  --classic-mode code_thought_coarse_sr2_front_slot_qd \
  --eoh-mode code_thought_coarse_sr2_front_slot_qd \
  --unified-mode code_thought_coarse_sr2_front_slot_qd \
  --require-full-subset
...
uv run python scripts/validate_pareto_front_run.py \
  ...
  --subset-config docs/journal_features/revamp_history/20260622_010615_KST_useful_bd_push/techniques/T55_coarse_sr2_front_slot_qd/tables/hard_tuning_subset.yaml \
  --classic-mode code_thought_coarse_sr2_front_slot_qd \
  --pareto-qd-mode code_thought_coarse_sr2_front_slot_qd \
  --require-full-subset
```
...
uv run python scripts/package_t48_gated_probe.py \
  ...
  --qd-root "${RUN_ROOT}/code_thought_coarse_sr2_front_slot_qd" \
  ...
  --output-dir docs/journal_features/revamp_history/20260622_010615_KST_useful_bd_push/techniques/T55_coarse_sr2_front_slot_qd/hard_tuning_package \
  --seed 1001 \
  --package-tag t55 \
  --package-title "T55 Coarse SR2 Front-Slot QD" \
  --qd-method code_thought_coarse_sr2_front_slot_qd \
  --qd-label "T55 coarse SR2 front-slot QD" \
  --counter-stem operator_counters \
  --counter-title "Operator Counters" \
  --counter-keys success_parent_requests,front_slot_lane_parent_requests,front_slot_lane_parent_hits,two_parent_attempts
```
