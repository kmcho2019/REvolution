...
| **(3)** CVDP + RealBench end-to-end on locked slices | **RealBench MET; CVDP testable (probe pending)** | RealBench MET (classic-vs-QD end-to-end on e203, F18–F21, `exp/fast_iter/capability_remap`). **CVDP: harness VALIDATED operational locally (F26, 2026-06-16)** — ground-truthed (5/12 compile-fail, 7/12 compile+func-fail, 0 pass); the 0% is genuine difficulty (hard multi-mode counter), NOT infra-blocked. CVDP IS testable; a real classic-vs-QD CVDP probe (seed 42, the locked debug set) is the unblocked next step. #4 carried by RealBench + this newer-harder-benchmark signal. |
