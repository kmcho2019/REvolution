...
| **(3)** CVDP + RealBench end-to-end on locked slices | **MET (both; QD never wins)** | RealBench: genuine capability ceiling on the 5 larger e203 modules (0 valid, all arms); on the 2 smallest classic 4 >= V2 3 > qd_target 2 (F18-21, F28; deepseek also 0 on large). **CVDP: the initial 0/9 was an interface-wiring confound (F30, FIXED — input.context dropped from the prompt); with the fix the LLM solves 9/10 easy tasks and classic = V2 = 9/10 (F31) — QD adds no value in the capable regime.** So #4 is answered by two harder benchmarks with our chosen method tested on both: QD ties where the LLM is capable (CVDP/small), is capped where it is not (RealBench-large); never wins. |
| **(4)** Manuscript per branch + 4 case-study artifacts | **DRAFTED (author to finish)** | Full core prose DRAFTED in `manuscript_artifacts/` (Intro, Methods, Results, Discussion, Conclusion, Abstract, Contributions) + reproducible Tables 1–2 + 2 of 4 case-study figures; QA-verified consistent. Author-owned to finish: Related Work (citations), the 2 bespoke figures, Overleaf assembly + the four-persona sign-off. |
