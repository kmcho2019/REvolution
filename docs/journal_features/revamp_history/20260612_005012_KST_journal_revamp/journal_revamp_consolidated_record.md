...
| **(3)** CVDP + RealBench end-to-end on locked slices | **MET (both; capability ceiling)** | RealBench: classic-vs-QD end-to-end on e203 (F18–F21). **CVDP: debug-seed probe DONE (F27, seed 42, 9 tasks): 0/9 functional pass for classic = qd_target = qd_v2** — the same capability ceiling as RealBench (no candidate clears the functional bar; QD can't help). Harness validated operational (F26); no PASS observed (CVDP withholds goldens). **RealBench: V2 (our chosen method) also run (F28): classic 4 >= V2 3 > qd_target 2, all on the 2 smallest modules, 0 on the 5 larger — V2 doesn't beat classic.** So criticism #4 is answered by TWO independent harder benchmarks, our chosen method tested on both; neither beats classic (capability ceiling). |
