...
| **(3)** CVDP + RealBench end-to-end on locked slices | **RealBench MET; CVDP SCOPED OUT** | RealBench MET (classic-vs-QD end-to-end on e203, F18–F21, `exp/fast_iter/capability_remap`). **CVDP end-to-end SCOPED OUT (2026-06-16)** — only smoke runs (`exp/codeevolve_cvdp_*_smoke`); same confirmatory class as held-out, and the #4 answer is carried by RealBench, so documented as a limitation rather than run. |
