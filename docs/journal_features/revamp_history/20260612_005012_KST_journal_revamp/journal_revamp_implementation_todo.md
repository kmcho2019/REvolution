...
- [ ] P1: Screen repair candidates one factor at a time on the fast subset: R-D k=2 (RUNNING, exp/fast_iter/k_ablation_k2), R-B failure feedback (--qd_operator_fail_feedback_chars 1200 + journal_thought_only_failfb profile; QUEUED behind k2 via exp/queue_after_k2.sh), R-A' spec-first realization template variant, R-C collapsed-axis warmup fallback (engine change; also the P2 coupling).
