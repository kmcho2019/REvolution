...
- [ ] P1: Screen repair candidates one factor at a time (fast subset for always-on mechanisms; data/configs/failure_regime_screen_subset.yaml for failure-path mechanisms - the v3 subset's high pass rates leave the fail pool empty, so R-B never fires there: observed 0 fail_pool parents): R-D k=2 (RUNNING, exp/fast_iter/k_ablation_k2), R-B failure feedback (--qd_operator_fail_feedback_chars 1200 + journal_thought_only_failfb profile; QUEUED behind k2 via exp/queue_after_k2.sh), R-A' spec-first realization template variant, R-C collapsed-axis warmup fallback (engine change; also the P2 coupling).
