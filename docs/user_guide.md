...
- Storage-saving example: `python scripts/archive_baseline.py --run-dir exp/<model> --archive-root baselines --artifact-mode candidate_core`

Each archive includes `manifest.json`, copied summary/report files, copied config snapshots, and `artifacts/raw_results.tar.xz`. Use `--artifact-mode full` (default) for complete raw artifacts, or `--artifact-mode candidate_core` to keep only candidate code/thought/feedback files in the tarball. The command is strict about reproducibility and fails if no `*_config.yaml|yml|json` snapshots are found under `--run-dir`.
