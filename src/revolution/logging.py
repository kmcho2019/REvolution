...
class EoHLogger:
    ...
    def __init__(
        ...
        self.all_format_passed: set[str] = set()
        ...
        self.total_llm_prompt_tokens: int = (
            0  # Track the input tokens used in LLM prompts
        )
        self.total_llm_completion_tokens: int = (
            0  # Track the output tokens generated by LLM completions
        )
        self.total_llm_code_prompt_tokens: int = (
            0  # Track the input tokens used in LLM code prompts
        )
        self.total_llm_code_completion_tokens: int = (
            0  # Track the output tokens generated by LLM code completions
        )
        self.total_llm_feedback_prompt_tokens: int = (
            0  # Track the input tokens used in LLM feedback prompts
        )
        self.total_llm_feedback_completion_tokens: int = (
            0  # Track the output tokens generated by LLM feedback completions
        )
    ...
    def log_generation(
        ...
        llm_code_prompt_tokens: int,
        llm_code_completion_tokens: int,
        llm_feedback_prompt_tokens: int,
        llm_feedback_completion_tokens: int,
        llm_stat_dict: dict[str, int],
        ...
        :param llm_prompt_tokens:                  Total number of prompt tokens used in this generation.
        :type llm_prompt_tokens: int
        :param llm_completion_tokens:              Total number of completion tokens used in this generation.
        :type llm_completion_tokens: int
        :param llm_code_prompt_tokens:             Total number of prompt tokens used in LLM code generation.
        :type llm_code_prompt_tokens: int
        :param llm_code_completion_tokens:         Total number of completion tokens used in LLM code generation.
        :type llm_code_completion_tokens: int
        :param llm_feedback_prompt_tokens:         Total number of prompt tokens used in LLM feedback generation.
        :type llm_feedback_prompt_tokens: int
        :param llm_feedback_completion_tokens:     Total number of completion tokens used in LLM feedback generation.
        :type llm_feedback_completion_tokens: int
        :param llm_stat_dict:                      Dictionary containing LLM API stats for this generation.
        :type llm_stat_dict: dict[str, int]
        ...
        # Calculate diff statistics for this generation
        ...
        total_format_success = sum(1 for c in candidates_this_gen if c.status != "failed_format")
        total_diff_success   = sum(1 for c in candidates_this_gen if c.status not in ["failed_format", "failed_diff"])
        total_syntax_success = sum(
            1 for c in candidates_this_gen if c.status not in ["failed_format", "failed_diff", "failed_syntax"]
        )
        total_func_success = sum(
            1
            for c in candidates_this_gen
            if c.status not in ["failed_format", "failed_diff", "failed_syntax", "failed_functionality"]
        )
        ...
        log_entry = {
            ...
            "status_counts_this_generation": dict(status_counts), 
            "diff_stats": {  
        ...
        self.total_llm_code_prompt_tokens += llm_code_prompt_tokens
        self.total_llm_code_completion_tokens += llm_code_completion_tokens
        self.total_llm_feedback_prompt_tokens += llm_feedback_prompt_tokens
        self.total_llm_feedback_completion_tokens += llm_feedback_completion_tokens
        ...
        for c in candidates_this_gen:
            ...
            if c.status != "failed_format":
                self.all_format_passed.add(c.id)
            if c.status not in ["failed_format", "failed_syntax"]:
                self.all_syntax_passed.add(c.id)
            if c.status not in ["failed_format", "failed_syntax", "failed_functionality"]:
    ...
    def finalize_summary(
        ...
        acc_rates = {"format": 0.0, "diff": 0.0, "syntax": 0.0, "functionality": 0.0, "synthesis_ppa": 0.0}
        if total_unique_generated > 0:
            acc_rates["format"] = len(self.all_format_passed) / total_unique_generated
            acc_rates["diff"]         = 1.0 - (self.total_diff_failed / max(1, total_unique_generated))
        ...
        summary_data = {
            ...
            "total_llm_code_prompt_tokens": self.total_llm_code_prompt_tokens,
            "total_llm_code_completion_tokens": self.total_llm_code_completion_tokens,
            "total_llm_feedback_prompt_tokens": self.total_llm_feedback_prompt_tokens,
            "total_llm_feedback_completion_tokens": self.total_llm_feedback_completion_tokens,
            ...
            "accumulated_diff_stats": {  
                "attempts": self.total_diff_attempts,
                "failed": self.total_diff_failed,
                "pass_rate": (
