...
class QDBudgetSplit:
    """Resolved per-generation QD budget allocation across QD search phases."""

    total_budget: int
    target_cells: int
    occupied_cells: int
    fail_share: float
    coverage_fail_share: float | None
    fail_share_cap: float | None
    fail_budget: int
    success_budget: int
    phase: QDBudgetPhase
    seed_budget: int
...
def split_qd_budget(
    *,
    ...
    fail_share = coverage_fail_share
    capped_fail_share = min(1.0, max(0.0, float(fail_share_cap)))
    fail_share = min(fail_share, capped_fail_share)
    ...
    if archive_empty:
        return QDBudgetSplit(
            ...
            fail_share_cap=capped_fail_share,
