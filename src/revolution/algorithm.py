...
class Heuristic:
    ...
    :param diff_apply_phase: Last diff-apply phase used for this candidate (if any).
    :type diff_apply_phase: str | None
    :param diff_apply_reason_code: Structured diff failure reason code (if any).
    :type diff_apply_reason_code: str | None
    """
...
class EoHEngine:
    ...
    def __init__(
        ...
        strategy_selection_method: StrategySelectionMethod = "ucb",
        epsilon: float = 0.1,
        ucb_c: float = 2.0,
        generation_mode: Literal["whole", "diff"] = "whole",
        diff_apply_policy: DiffApplyPolicy = "hybrid",
        ...
        self.diff_max_tokens: int = diff_max_tokens
        self.diff_compact_context: bool = diff_compact_context
        ...
        self.diff_fuzzy_margin: float = diff_fuzzy_margin
    ...
    def _format_parent_for_prompt(
        ...
        if include_code:
            parent_payload["code"] = code_override if code_override is not None else parent.code
        elif self.diff_compact_context:
            parent_payload["code"] = "<omitted in compact diff context; see original_file>"
    ...
    def _build_prompt_request(
        ...
        if mode == "diff":
            request["max_tokens"] = self.diff_max_tokens
        return request
    ...
    def _reset_diff_apply_diagnostics(self) -> None:
        self._last_diff_apply_diagnostics = {
            ...
            "reason": None,
            "phase": None,
            "matching_policy": self.diff_apply_policy,
            "hunks": [],
        }
    ...
    def _parse_diff_block(self, diff_text: str) -> Iterator[tuple[str, str, str]]:
        ...
        i = 0
        while i < len(lines):
            # Skip spacing/noise lines between blocks.
    ...
    def _do_replace(self, content: str, original: str, updated: str, filename: str | None = None) -> str | None:
        """
        Perform a deterministic SEARCH/REPLACE pipeline on `content`:
        A) strict exact match
        B) whitespace-normalized line match (hybrid/fuzzy only)
        C) wildcard/fuzzy fallback (hybrid/fuzzy only)
        ...
        # Phase A: strict exact unique match first (fast path)
        ...
        if exact_count > 1 and self.diff_apply_policy == "strict":
            self._set_replace_diag(
        ...
        # strict policy stops after phase A
        if self.diff_apply_policy == "strict":
            self._set_replace_diag(
        ...
        # Phase B: controlled whitespace-normalized matching
        ...
        # Phase C1: optional wildcard '...' path
        if self.diff_allow_dots:
            try:
        ...
        # Phase C2: guarded fuzzy fallback
    ...
    def _normalize_ws_line(self, line: str) -> str:
        # Controlled normalization: ignore horizontal whitespace drift only.
    ...
    def _replace_closest_edit_distance(
        ...
        if best_ratio < self.diff_similarity_threshold:
            return None, {
                ...
                "reason": (
                    ...
                    f"{self.diff_similarity_threshold:.4f}"
                ),
            }

        if len(ranked) > 1 and (best_ratio - second_ratio) < self.diff_fuzzy_margin:
            return None, {
                "best_ratio": best_ratio,
                "second_ratio": second_ratio,
                "top_candidates": top_candidates,
                ...
                "reason": (
                    ...
                    f"is below required margin {self.diff_fuzzy_margin:.4f}"
                ),
            }
    ...
    def _replace_most_similar_chunk(
        ...
        *,
        ...
        if allow_fuzzy:
            res, diag = self._replace_closest_edit_distance(
                whole_lines, part, part_lines, replace_lines
            )
            if res:
                self._set_replace_diag(
        ...
        self._set_replace_diag(
            phase="strict_no_match",
            reason_code="search_not_found",
            reason="strict diff policy did not find a unique match",
        )
    ...
    def _apply_json_edits(
        ...
        :rtype: str | None
        """
        self._last_diff_apply_diagnostics["matching_policy"] = self.diff_apply_policy
        ...
        if target_file_path:
            ...
            if len(edits) != 1:
                ...
                return None
            if not isinstance(edits[0], dict):
                self._set_diff_failure(
                    "invalid_json_diff",
                    "JSON diff payload contains a non-object edit entry.",
                    phase="json_validate",
                )
                return None
        ...
        if not chosen_edits:
            self._set_diff_failure(
                "invalid_json_diff",
                "JSON diff payload did not provide any valid edit entries.",
                phase="json_validate",
            )
            return None
        ...
        for hunk_idx, h in enumerate(hunks):
            ...
            # Preflight overlap/conflict detection for exact-unique anchors.
            if search.strip():
        ...
        for hunk_idx, hunk in enumerate(normalized_hunks):
            ...
            file_hint = chosen_file or target_file_path
            result = self._do_replace(new_content, search, replace, filename=file_hint)
            if result is None:
        ...
        try:
            ...
            if isinstance(obj, dict):
                ...
                if isinstance(obj.get("code"), dict) and "edits" in obj["code"]:
                    # Accept full eoh_v1 envelope where edits live in code.edits.
                    edits_obj = cast(dict[str, Any], obj["code"])
                if "edits" in edits_obj:
                    updated = self._apply_json_edits(original_content, edits_obj, target_file_path)
            ...
            if parent_file and os.path.exists(parent_file):
                try:
                    with open(parent_file, "rb") as pf:
                        parent_digest = hashlib.sha256(pf.read()).hexdigest()
                except Exception:
                    parent_digest = None
            meta = {
                "timestamp_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
    ...
    def initialize_population(self) -> None:
        ...
        if len(results_with_meta) < self.population_size:
            missing = self.population_size - len(results_with_meta)
            print(
                f"WARNING: LLM returned {len(results_with_meta)}/{self.population_size} "
                f"initial responses. Padding {missing} missing entries as failed_format."
            )
            for _ in range(missing):
...
class SingleShotEngine(EoHEngine):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            diff_apply_policy=diff_apply_policy,
            diff_max_tokens=diff_max_tokens,
            diff_compact_context=diff_compact_context,
            diff_similarity_threshold=diff_similarity_threshold,
            diff_fuzzy_margin=diff_fuzzy_margin,
