...
from scripts.package_t46_t11_runtime_pca4_graph import METHODS, PROBLEMS, main


def test_package_t46_t11_runtime_pca4_graph(tmp_path: Path) -> None:
    roots = {
        "t46": tmp_path / "t46",
        "t45": tmp_path / "t45",
        "t44": tmp_path / "t44",
    ...
    rows = list(csv.DictReader((output_dir / "tables" / "t46_candidate_ppa_points.csv").open()))
    summary = list(
        csv.DictReader((output_dir / "tables" / "t46_problem_method_summary.csv").open())
    )
    assert len(rows) == len(METHODS) * len(PROBLEMS) * 2
    assert len(summary) == len(METHODS) * len(PROBLEMS)
    assert (output_dir / "figures" / "t46_raw_area_power_fronts.png").read_bytes().startswith(
        b"\x89PNG"
    )
    html = (output_dir / "visualizations" / "direct_ppa_pareto" / "index.html").read_text(
        encoding="utf-8"
    )
    assert "T46 Direct PPA Fronts" in html
    assert "t46_raw_area_power_fronts.png" in html
