...
from scripts.package_t43_staged_sparse_yield_gate import METHODS, PROBLEMS, main


def test_package_t43_staged_sparse_yield_gate(tmp_path: Path) -> None:
    roots = {
        "t43": tmp_path / "t43",
        "t42": tmp_path / "t42",
        "t41": tmp_path / "t41",
    ...
    rows = list(csv.DictReader((output_dir / "tables" / "t43_candidate_ppa_points.csv").open()))
    summary = list(
        csv.DictReader((output_dir / "tables" / "t43_problem_method_summary.csv").open())
    )
    assert len(rows) == len(METHODS) * len(PROBLEMS) * 2
    assert len(summary) == len(METHODS) * len(PROBLEMS)
    assert (output_dir / "figures" / "t43_raw_area_power_fronts.png").read_bytes().startswith(
        b"\x89PNG"
    )
    html = (output_dir / "visualizations" / "direct_ppa_pareto" / "index.html").read_text(
        encoding="utf-8"
    )
    assert "T43 Direct PPA Fronts" in html
    assert "t43_raw_area_power_fronts.png" in html
