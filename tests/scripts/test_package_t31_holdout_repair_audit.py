...
from scripts.package_t31_holdout_repair_audit import (
    T30_CLASSIC_MODE,
    T30_T26_MODE,
    T31_MODE,
    main,
...
def test_package_t31_holdout_repair_audit(tmp_path: Path) -> None:
    ...
    _write_validation(t31_run_root)
    ...
    live_rows = list(
        csv.DictReader(
            (output_dir / "tables" / "t31_holdout_live_problem_metrics.csv").open()
        )
    )
    family_rows = list(
        csv.DictReader(
            (output_dir / "tables" / "t31_holdout_family_aggregate_metrics.csv").open()
        )
    )
    t31 = next(row for row in family_rows if row["method"] == "sr_raw_fail_feedback_repair_qd")
    assert len(live_rows) == 9
    assert int(t31["valid_ppa_count"]) == len(PROBLEMS) * 2
    assert (output_dir / "tables" / "t31_holdout_pareto_validation.json").is_file()
    assert (output_dir / "figures" / "t31_holdout_live_aggregate.png").read_bytes().startswith(
        b"\x89PNG"
    )
    assert (
        output_dir / "figures" / "t31_holdout_ppa_pareto_area_power.png"
    ).read_bytes().startswith(b"\x89PNG")
    assert (
        output_dir / "figures" / "t31_holdout_ppa_pareto_area_power_candidate_zoom.png"
    ).read_bytes().startswith(b"\x89PNG")
