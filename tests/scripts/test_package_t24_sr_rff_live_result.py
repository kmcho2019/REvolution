...
def test_package_t24_sr_rff_live_result(tmp_path: Path) -> None:
    ...
    csv_path = output_dir / "live_sr_rff_vs_classic.csv"
    figure_path = output_dir.parent / "figures" / "live_sr_rff_vs_classic.png"
    rows = list(csv.DictReader(csv_path.read_text(encoding="utf-8").splitlines()))
    assert len(rows) == 3
    assert rows[0]["best_score_delta"] == "0.250000"
    assert rows[0]["sr_rff_archive_members"] == "3"
    assert rows[0]["sr_rff_global_pareto_members"] == "2"
        ...
        json.dumps(
            {
                ...
                "accumulated_success_rates": {"synthesis_ppa": synthesis_rate},
            }
        ),
        encoding="utf-8",
    )
