...
from scripts.package_t41_adaptive_sparse_yield_gate import METHODS, PROBLEMS, main


def test_package_t41_adaptive_sparse_yield_gate(tmp_path: Path) -> None:
    roots = {
        "t41": tmp_path / "t41",
        "t40": tmp_path / "t40",
        "t39": tmp_path / "t39",
    ...
    rows = list(csv.DictReader((output_dir / "tables" / "t41_candidate_ppa_points.csv").open()))
    summary = list(
        csv.DictReader((output_dir / "tables" / "t41_problem_method_summary.csv").open())
    )
    assert len(rows) == len(METHODS) * len(PROBLEMS) * 2
    assert len(summary) == len(METHODS) * len(PROBLEMS)
    assert (output_dir / "figures" / "t41_raw_area_power_fronts.png").read_bytes().startswith(
        b"\x89PNG"
    )
    html = (output_dir / "visualizations" / "direct_ppa_pareto" / "index.html").read_text(
        encoding="utf-8"
    )
    assert "T41 Direct PPA Fronts" in html
    assert "t41_raw_area_power_fronts.png" in html
