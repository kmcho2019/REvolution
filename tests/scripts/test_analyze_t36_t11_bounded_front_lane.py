...
_SCRIPT = Path(__file__).resolve().parents[2] / "scripts" / "analyze_t36_t11_bounded_front_lane.py"
_SPEC = importlib.util.spec_from_file_location("analyze_t36_t11_bounded_front_lane", _SCRIPT)
assert _SPEC is not None and _SPEC.loader is not None
mod = importlib.util.module_from_spec(_SPEC)
sys.modules.setdefault("analyze_t36_t11_bounded_front_lane", mod)
_SPEC.loader.exec_module(mod)


def test_analyze_t36_writes_bounded_front_lane_artifacts(tmp_path: Path) -> None:
    ...
    assert "t36_top64_lane_10" in set(comparison["representation"])
    lane_summary = pd.read_csv(package_dir / "tables" / "lane_summary.csv")
    assert set(lane_summary["front_lane_quota"]) == {0.05, 0.1, 0.15, 0.2}
    assert set(lane_summary["front_lane_slots"]) == {1}
    assert (package_dir / "figures" / "t36_multi_problem_ppa_pareto_fronts.png").read_bytes().startswith(
        b"\x89PNG"
    )
    viewer = package_dir / "visualizations" / "direct_ppa_pareto"
    assert (viewer / "index.html").is_file()
    assert "T36 Direct Raw PPA Pareto Front" in (viewer / "index.html").read_text(encoding="utf-8")
