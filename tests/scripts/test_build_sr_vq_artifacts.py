...
_SCRIPT = (
    ...
    / "scripts"
    / "build_sr_vq_artifacts.py"
)
_SPEC = importlib.util.spec_from_file_location("build_sr_vq_artifacts", _SCRIPT)
assert _SPEC is not None and _SPEC.loader is not None
mod = importlib.util.module_from_spec(_SPEC)
sys.modules.setdefault("build_sr_vq_artifacts", mod)
_SPEC.loader.exec_module(mod)


def test_build_artifacts_writes_frozen_sr_vq_files(tmp_path: Path) -> None:
    ...
    artifact_path = output_dir / "sr_vq_codebook_artifact.json"
    saved = json.loads(artifact_path.read_text(encoding="utf-8"))
    regions = pd.read_parquet(output_dir / "codebook_regions.parquet")
    ...
    assert saved["descriptor_version"] == "sr_vq_codebook_v1"
    assert saved["codebook_size"] == 3
    ...
    assert set(vectors["descriptor_version"]) == {"sr_vq_codebook_v1"}
