...
_SCRIPT = Path(__file__).resolve().parents[2] / "scripts" / "analyze_t35_t11_pareto_coupling.py"
_SPEC = importlib.util.spec_from_file_location("analyze_t35_t11_pareto_coupling", _SCRIPT)
assert _SPEC is not None and _SPEC.loader is not None
mod = importlib.util.module_from_spec(_SPEC)
sys.modules.setdefault("analyze_t35_t11_pareto_coupling", mod)
_SPEC.loader.exec_module(mod)


def test_analyze_t35_writes_pareto_coupling_artifacts(tmp_path: Path) -> None:
    ...
    assert "t35_top64_cell_pareto" in set(comparison["representation"])
    assert "selected_all_valid_front_hits" in comparison.columns
    assert (package_dir / "figures" / "t35_multi_problem_ppa_pareto_fronts.png").read_bytes().startswith(
        b"\x89PNG"
    )
