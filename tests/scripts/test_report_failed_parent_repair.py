...
def test_report_validates_and_pairs_fresh_arms(tmp_path: Path) -> None:
    ...
    _write_arm(classic, treatment=False)
    _write_arm(treatment, treatment=True)

    summary = report.generate_report(classic, treatment, output, 42, ["Prob001"], 2, 1)
...
def test_report_rejects_uninstrumented_classic(tmp_path: Path) -> None:
    ...
    with pytest.raises(AssertionError):
        report.generate_report(
            classic, treatment, tmp_path / "report", 42, ["Prob001"], 2, 1
        )
