...
def plot_all_individuals(all_data, ref_power, ref_area, num_generations, cmap, max_area=None, max_power=None):
    ...
    plt.tight_layout(); plt.savefig("plot1_all_individuals.png", dpi=300)
    print("Generated plot1_all_individuals.png")

def plot_generational_pareto_fronts(all_data, ref_power, ref_area, num_generations, cmap, max_area=None, max_power=None):
    ...
    apply_axis_limits(ax, max_area, max_power)
    plt.tight_layout(); plt.savefig("plot2_generational_fronts.png", dpi=300)
    print("Generated plot2_generational_fronts.png")

def plot_grouped_pareto_fronts(all_data, ref_power, ref_area, groups, group_labels, title, filename, max_area=None, max_power=None, cmap_name='viridis'):
...
def plot_highlighted_top_scorers(all_data, ref_power, ref_area, groups, group_labels, title, filename, max_area=None, max_power=None, cmap_name='magma'):
    ...
    for i, (group_range, group_label) in enumerate(zip(groups, group_labels)):
        ...
        ax.scatter([p['area'] for p in group_points], [p['power'] for p in group_points], color=cmap(i), alpha=0.1, s=40, label=f'{group_label} (all)')
        
        # Highlight top 5 scorers
        if top_scorers:
            top_areas = [p['area'] for p in top_scorers]
            top_powers = [p['power'] for p in top_scorers]
            ax.scatter(top_areas, top_powers, marker='D', s=150, color=cmap(i), edgecolor='white', linewidth=1, zorder=4, label=f'{group_label} (Top 5)')
...
def main():
    ...
    parser.add_argument("--summary_json", required=True, help="Path to the summary JSON file (*_summary.json).")
    parser.add_argument("--max_area", type=float, help="Set the maximum x-axis (Area) cutoff for all plots.")
    parser.add_argument("--max_power", type=float, help="Set the maximum y-axis (Power) cutoff for all plots.")
    ...
    # --- Original Plots ---
    plot_all_individuals(all_data, ref_power, ref_area, num_generations, cmap_generations, args.max_area, args.max_power)
    plot_generational_pareto_fronts(all_data, ref_power, ref_area, num_generations, cmap_generations, args.max_area, args.max_power)
    ...
    if num_generations > 4:
        ...
        plot_grouped_pareto_fronts(all_data, ref_power, ref_area, groups, group_labels, 'Pareto Fronts (4 Equal Groups)', 'plot3_equal_groups.png', args.max_area, args.max_power, 'plasma')
        # --- NEW PLOT 5 ---
        plot_highlighted_top_scorers(all_data, ref_power, ref_area, groups, group_labels, 'Top 5 Scorers (4 Equal Groups)', 'plot5_top5_equal_groups.png', args.max_area, args.max_power, 'magma')
    ...
    if num_generations > 13:
        ...
        plot_grouped_pareto_fronts(all_data, ref_power, ref_area, groups_4, labels_4, 'Pareto Fronts (Custom Groups 1)', 'plot4_custom_groups1.png', args.max_area, args.max_power, 'cividis')
        # --- NEW PLOT 6 ---
        plot_highlighted_top_scorers(all_data, ref_power, ref_area, groups_4, labels_4, 'Top 5 Scorers (Custom Groups 1)', 'plot6_top5_custom_groups1.png', args.max_area, args.max_power, 'inferno')

    if num_generations > 12:
        # --- NEW PLOT 7 ---
        groups_3 = [(0, 4), (5, 11), (12, num_generations - 1)]
        labels_3 = [f'Gens {g[0]}-{g[1]}' for g in groups_3]
        plot_highlighted_top_scorers(all_data, ref_power, ref_area, groups_3, labels_3, 'Top 5 Scorers (Custom Groups 2)', 'plot7_top5_custom_groups2.png', args.max_area, args.max_power, 'viridis')
