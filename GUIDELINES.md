...
- Journal-revamp onboarding (read in this order when implementing the TCAD goal):
  ...
  [docs/journal_features/13_findings_dashboard.md](docs/journal_features/13_findings_dashboard.md) is the START-HERE current-state view (topic-indexed findings with stable IDs F1–F6/M1–M6, result tables, open decisions, navigation map — refresh its tables as runs land);
