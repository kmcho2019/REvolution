module TopModule (
  ...
  // Output Logic
  assign done = state == 10'b0100000000 ? 1'b1 : 1'b0; // done only in Wait state
  ...
  assign S_next = (state == 10'b0000000001) ? 1'b1 : 1'b0; // Next state is S from B3 or Wait
  assign S1_next = (state == 10'b0000000010) ? 1'b1 : 1'b0; // Next state is S1 from S
