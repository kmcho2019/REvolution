...
always @(posedge clk) begin
    if (reset) begin
      ...
      if (ena[1])
          q_tens <= (q_tens == 4'b1001) ? 4'b0000 : (ena[1]) ? q_tens + 1 : q_tens;
      
      // Increment hundreds digit if enable for hundreds is high
      if (ena[2])
          q_hundreds <= (q_hundreds == 4'b1001) ? 4'b0000 : (ena[2]) ? q_hundreds + 1 : q_hundreds;
      
      // Increment thousands digit if enable for thousands is high
      if (ena[3])
          q_thousands <= (q_thousands == 4'b1001) ? 4'b0000 : (ena[3]) ? q_thousands + 1 : q_thousands;
    end
end
