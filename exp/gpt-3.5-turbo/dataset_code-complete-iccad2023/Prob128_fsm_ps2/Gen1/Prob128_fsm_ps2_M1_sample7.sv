...
always @(*) begin
  ...
  case (state)
    ...
    FIRST_BYTE: begin
      if (in[3] == 0) begin
        next_state = IDLE; // If the first byte is incorrect, restart
      end else begin
    ...
    SECOND_BYTE: begin
      if (in[3] == 0) begin
        next_state = IDLE; // If the second byte is incorrect, restart
      end else begin
    ...
    THIRD_BYTE: begin
      next_state = IDLE; // Reset after the third byte
    end
  endcase
end
