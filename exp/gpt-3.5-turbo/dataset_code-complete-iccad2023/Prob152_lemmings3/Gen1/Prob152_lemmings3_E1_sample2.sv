...
always @* begin
    case (state)
        WALK_LEFT: begin
            walk_left = 1;
            walk_right = 0;
            aaah = !ground && state != FALLING;
            digging = 0;
            next_state = bump_right ? WALK_RIGHT : (dig && ground) ? DIGGING : (!ground) ? FALLING : WALK_LEFT;
        end
        WALK_RIGHT: begin
            walk_left = 0;
            walk_right = 1;
            aaah = !ground && state != FALLING;
            digging = 0;
            next_state = bump_left ? WALK_LEFT : (dig && ground) ? DIGGING : (!ground) ? FALLING : WALK_RIGHT;
        end
        FALLING: begin
