...
always @* begin
    ...
    // Correct state transitions for Lemming hitting ground after falling or finishing digging
    if (ground && ((state == FALLING) || (state == DIGGING))) begin
        next_state = state == DIGGING ? (walk_left ? WALK_LEFT : WALK_RIGHT) : (walk_left ? WALK_LEFT : WALK_RIGHT);
    end
end
