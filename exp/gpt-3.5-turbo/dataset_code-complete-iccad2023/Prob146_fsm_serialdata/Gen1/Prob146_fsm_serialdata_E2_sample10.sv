      ...
      case (state)
        ...
        3'b010: begin // Stop bit expected
          if (in == 1'b1) begin
            ...
            state <= 3'b011; // Transition to synchronization state
          end
        end
        3'b011: begin // Synchronization state, waiting for start bit of next byte
          if (in == 1'b0) begin
            state <= 3'b001; // Transition back to start bit detected state
          end
        end
      endcase
    end
