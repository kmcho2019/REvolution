#!/usr/bin/env python3
"""Package the T32 front-preserving emitter holdout audit."""
...
T32_MODE = "sr_raw_front_preserving_emitter_qd/seed_1001/openai_gpt-oss-120b"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--t32-run-root", required=True, type=Path)
    ...
    methods = build_methods(args.t30_run_root, args.t31_run_root, args.t32_run_root)
    old_methods = holdout.METHODS
    old_prefix = holdout.OUTPUT_PREFIX
    old_title = holdout.FIGURE_TITLE_PREFIX
    holdout.METHODS = methods
    holdout.OUTPUT_PREFIX = "t32_holdout"
    holdout.FIGURE_TITLE_PREFIX = "T32 Holdout"
    try:
        live_rows = [
        ...
        write_csv(table_dir / "t32_holdout_live_problem_metrics.csv", live_rows)
        write_csv(table_dir / "t32_holdout_live_aggregate_metrics.csv", live_aggregate_rows)
        write_csv(table_dir / "t32_holdout_live_comparison_deltas.csv", live_delta_rows)
        write_csv(table_dir / "t32_holdout_family_candidate_rows.csv", holdout.candidate_rows(candidates))
        write_csv(table_dir / "t32_holdout_family_problem_metrics.csv", family_rows)
        write_csv(table_dir / "t32_holdout_family_aggregate_metrics.csv", family_aggregate_rows)
        write_csv(table_dir / "t32_holdout_family_comparison_deltas.csv", family_delta_rows)
        write_csv(table_dir / "t32_holdout_method_manifest.csv", manifest_rows(methods))
        copy_validation(args.t32_run_root, table_dir)
...
def build_methods(t30_run_root: Path, t31_run_root: Path, t32_run_root: Path) -> tuple[dict[str, str], ...]:
    t30_root = t30_run_root.resolve()
    t31_root = t31_run_root.resolve()
    t32_root = t32_run_root.resolve()
    return (
        {
        ...
        {
            "method": "sr_raw_front_preserving_emitter_qd",
            "label": "T32 front-preserving emitter",
            "mode": str(t32_root / T32_MODE),
            "color": "#b07aa1",
        },
    )
...
def comparison_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for method in (
        ...
        "sr_raw_front_preserving_emitter_qd",
    ):
...
def copy_validation(t32_run_root: Path, table_dir: Path) -> None:
    json_path = t32_run_root / "pareto_front_validation.json"
    md_path = t32_run_root / "pareto_front_validation.md"
    assert json_path.is_file()
    assert md_path.is_file()
    shutil.copyfile(json_path, table_dir / "t32_holdout_pareto_validation.json")
    shutil.copyfile(md_path, table_dir / "t32_holdout_pareto_validation.md")
