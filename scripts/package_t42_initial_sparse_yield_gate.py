#!/usr/bin/env python3
"""Package the T42 initial sparse-yield gate screen."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    write_csv(table_dir / "t42_candidate_ppa_points.csv", candidate_rows(candidates))
    write_csv(table_dir / "t42_problem_method_summary.csv", summary_rows)
    write_csv(table_dir / "t42_method_manifest.csv", manifest_rows(roots, METHODS))
    plot_raw_fronts(
        candidates,
        figure_dir / "t42_raw_area_power_fronts.png",
        METHODS,
        "T42 Initial Sparse-Yield Gate",
    )
    plot_count_summary(
        summary_rows,
        figure_dir / "t42_front_count_summary.png",
        METHODS,
        "T42 Initial Sparse-Yield Gate",
    )
    write_viewer(
        ...
        summary_rows,
        title="T42 Direct PPA Fronts",
        image_name="t42_raw_area_power_fronts.png",
    )
